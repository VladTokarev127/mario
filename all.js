...
function applyNewJs() {
	...
	document.getElementById("paused") && document.getElementById("paused").remove(),
	document.getElementById("mobile-controls") && document.getElementById("mobile-controls").remove();
	var e = createElement("div", {
			id: "paused"
	})
		, t = createElement("span");
	t.innerText = "CLICK TO UNPAUSE",
	e.appendChild(t),
	body.appendChild(e);
	...
	c.addEventListener("touchstart", (function(e) {
			e.preventDefault(),
			triggerKeyboardEvent(document.body, 68, "keydown")
	}
	)),
	c.addEventListener("touchend", (function(e) {
			triggerKeyboardEvent(document.body, 68, "keyup")
	}
	)),
	m.addEventListener("touchstart", (function(e) {
			e.preventDefault(),
			triggerKeyboardEvent(document.body, 65, "keydown")
	}
	)),
	m.addEventListener("touchend", (function(e) {
			triggerKeyboardEvent(document.body, 65, "keyup")
	}
	)),
	h.addEventListener("touchstart", (function(e) {
			e.preventDefault(),
			triggerKeyboardEvent(document.body, 87, "keydown")
	}
	)),
	h.addEventListener("touchend", (function(e) {
			triggerKeyboardEvent(document.body, 87, "keyup")
	}
	)),
	f.addEventListener("touchstart", (function(e) {
			e.preventDefault(),
			triggerKeyboardEvent(document.body, 83, "keydown")
	}
	)),
	f.addEventListener("touchend", (function(e) {
			triggerKeyboardEvent(document.body, 83, "keyup")
	}
	)),
	g.addEventListener("touchstart", (function(e) {
			e.preventDefault(),
			triggerKeyboardEvent(document.body, 17, "keydown")
	}
	)),
	g.addEventListener("touchend", (function(e) {
			triggerKeyboardEvent(document.body, 17, "keyup")
	}
	)),
...
function resetData() {
	var e;
	(e = document.getElementById("data_display")) && body.removeChild(e),
...
function setDataDisplay() {
	...
	for (var n in body.appendChild(e),
	data.display = e,
	t)
			e.appendChild(data[t[n]].element),
			updateDataElement(data[t[n]]);
	body.appendChild(data.display),
	is_mobile && applyNewJs()
}
...
function updateDataTime(e) {
	1 != e.dir && (100 == e.amount ? playCurrentThemeHurry() : e.amount <= 0 && killPlayer(player, !0)),
	notime || (map.time = e.amount += e.dir,
	updateDataElement(e))
}
...
function score(e, t, n) {
	if (!(t <= 0)) {
			if (1 == arguments.length)
					return score(player, e);
			if (localStorage.highscore = Math.max(localStorage.highscore, data.score.amount += t),
			console.log(e.level),
			console.log(data.score.amount),
			n) {
					var x = addText(t, e.left, e.top);
					x.yvel = -unitsized4,
					TimeHandler.addEvent(killScore, 49, x)
			}
			for (; data.score > 1e4; )
					gainLife(),
					data.score.amount = data.score.amount % 1e4;
			updateDataElement(data.score)
	}
}
function killScore(e) {
	body.contains(e) && body.removeChild(e),
	killNormal(e),
	deleteThing(e, texts, texts.indexOf(e))
}
function findScore(e) {
	return e < data.scorelevs.length ? data.scorelevs[e] : (gainLife(),
	-1)
}
...
function setEditorLibrary() {
	window.editor = {
			...
			solids: {
					...
					Brick: {
							...
							prefunc_custom: function(e, t, n, x) {
									var r = "Brick, ";
									return r += e.xloc + ", " + e.yloc,
									t.contents && (r += ", " + t.contents[0].name),
									r
							}
					},
					Block: {
							...
							followerUpdate: function(e, t) {
									var n = [];
									return "1Up Mushroom" == t.contents ? n.push([Mushroom, 1]) : n.push(window[t.contents]),
							...
							prefunc_custom: function(e, t, n, x) {
									var r = "Block, "
										...
										, a = i[0].name;
									return r += e.xloc + ", " + e.yloc,
									"Coin" != a ? ("Mushroom" == a && i[1] ? r += ", [Mushroom, " + String(i[1]) + "]" : r += ", " + a,
									t.hidden && (r += ", true")) : t.hidden && (r += ", false, true"),
									r
							}
					},
					Cannon: {
					...
					Pipe: {
							...
							followerUpdate: function(e, t) {
									var n = [];
									return n.push(Number(t.height)),
									n.push(Boolean(t.Pirhana)),
									n
							},
							sprite_source: "top"
					},
					...
					CastleBlock: {
							...
							followerUpdate: function(e, t) {
									return [[Number(t.fireballs), "CW" == t.direction], "True" == t.hidden]
							}
					},
					Springboard: {
							height: 14.5,
	...
	for (n in editor)
			...
			editor[n])
					t = e[x],
					proliferate(t, editor.defaults, !0);
	for (n in e = editor.scenery)
			t = e[n],
			proliferate(t, {
			...
			t.prefunc == pushPreThing && (t.prefunc = pushPreScenery)
}
...
function createEditorSidebar() {
	var e, t = ["Solids", "Characters", "Scenery", "Settings"], n = editor.sidebar = createElement("div", {
			id: "sidebar"
	...
	for (e in n.appendChild(x),
	x.appendChild(r),
	t)
			r.appendChild(createElement("option", {
					innerText: t[e]
			}));
	n.appendChild(i),
	body.appendChild(window.sidebar = n)
}
...
function createEditorScrollers() {
	...
	e = x.length - 1; e >= 0; --e)
			x[e],
			n = r[x[e]] = createElement("div", settings),
			t.appendChild(n);
	proliferate(r.left, {
			id: "left",
...
function editorScrollingStart(e) {
	var t = e.target.dx;
	editorPreventClicks(),
	editor.scrolling = TimeHandler.addEventInterval(editorScrolling, 1, 1 / 0, -t),
...
function editorScrolling(e) {
	if (scrollEditor(e),
	editor.xloc >= 0)
			return scrollEditor(-editor.xloc),
			editorScrollingStop(),
...
function createEditorGuideLines() {
	var e, t, n, x = {
			...
			jumplev1: jumplev1,
			jumplev2: jumplev2
	}, r = 16 * unitsize + "px", i = map.floor;
	for (e in window.maplines = t = document.createElement("div"),
	t.style.marginLeft = r,
	t.id = "maplines",
	x)
			n = createElement("div", {
					innerText: e,
...
function setEditorControls(e) {
	...
	var t, n, x, r = document.getElementById("controls"), i = createElement("div", {
			id: "controls"
	}), a = editor.controls = {
			container: i
	};
	for (x in r && (r.innerHTML = ""),
	e)
			t = e[x],
			n = createElement("div", {
					id: t,
...
function setEditorTriggers() {
	...
	for (e = t.length - 1; e >= 0; --e)
			t[e].onclick = editorMouseClick;
	document.onmousemove = editorFollowerFollowsCursor
}
function editorMouseClick(e) {
	if (window.editing && !editor.clicking) {
			if (editorPreventClicks(),
			editor.erasing)
					return editorPlaceEraser(e);
			if (!editor.in_settings && editor.canplace) {
					var t = editor.section_name
						, n = (window[t],
					editor.current_selected,
					editor.follower);
					editor.placed.push(n),
					editor.follower = !1,
					editorSetCurrentThingFromName(null, !0),
					paused && refillCanvas(),
					n.was_follower = !0,
					delete n.onclick,
					editor.playing && (thingRetrieveVelocity(n),
					proliferate(n, n.reference.attributes))
			}
	}
}
function editorSelectSection() {
	var e = (this || editor.sectionselect).value.toLowerCase();
	(editor.in_settings = "settings" == e) ? (editorSetSection(e, !0),
	editorSetSectionSettings()) : editorSetSection(e)
}
function editorSetSection(e, t) {
	var n, x, r = editor.section = editor[e], i = editor.bottombar, a = 0;
	if (editor.section_name = e,
	i.innerHTML = "",
	!t)
			for (e in r)
					++a,
					x = editorAddBottomPreview(i, e, r[e]),
					n || (n = x);
	a ? (i.style.visibility = "visible",
	editorSetCurrentThingFromCanvas(n)) : i.style.visibility = "hidden"
}
function editorAddBottomPreview(e, t, n) {
	var x, r = n.width, i = n.height, a = window[t], o = a ? ThingCreate(a, n.previewargs) : new Thing(Sprite,t), l = createElement("div", {
			width: r * unitsize + "px",
	...
	r * unitsizet2), u = i * unitsizet2, p = s.getContext("2d");
	return canvasDisableSmoothing(s),
	editor.bottombar.things[t] = s.thing = o,
	addClass(o, "editor"),
	x = o.canvas,
	o.canvases && (x = o.canvases[n.sprite_source || "middle"].canvas),
	n.previewsize ? (p.fillStyle = p.createPattern(x, "repeat"),
	p.fillRect(0, 0, d, u)) : p.drawImage(x, 0, 0, d, u),
	l.appendChild(l.canvas = s),
	e.appendChild(l),
	e[t] = l,
	s
}
function editorSetSectionSettings() {
	var e, t = editor.settings, n = "<table>";
	n += "<h3 class='title'>Settings</h3>",
	n += addArgumentOption("night", Boolean, t.night),
	n += addArgumentOption("setting", ["Overworld", "Underworld", "Underwater", "Castle", "Sky"], t.setting),
	n += addArgumentOption("alt", Boolean, t.alt),
	n += "</table>",
	options.innerHTML = n,
	ensureOptionsAboveZero(editorUpdateSettingsOption),
	e = editor.sidebar.getElementsByTagName("table")[0].rows,
	editor.settings.night_elem = e[0].cells[1].firstChild,
	editor.settings.setting_elem = e[1].cells[1].firstChild,
	editor.settings.alt_elem = e[2].cells[1].firstChild,
	editor.follower && killNormal(editor.follower),
	editor.follower = !1
}
...
function updateCurrentArguments(e, t) {
	t = t || {};
	var n, x = editor.options, r = "<table>", i = t.mydefaults || {}, a = t.arguments || {};
	for (n in r += "<h3 class='title'>" + e + "</h3>",
	a.width || (r += addStaticOption("width", t.width)),
	a.height || (r += addStaticOption("height", t.height)),
	a)
			r += addArgumentOption(n.replace("_", "-"), a[n], null, i);
	r += "</table>",
	x.innerHTML = r,
...
function addStaticOption(e, t) {
	return t == 1 / 0 && (t = "Inf."),
	"<tr id='option_" + e + "' class='auto'><td>" + e + ": </td><td class='auto'>" + t + "</td></tr>"
}
...
function ensureOptionsAboveZero(e) {
	...
	for (i = n.length - 1; i >= 0; --i)
			(t = n[i]).onchange = t.onclick = t.onkeypress = editorInputEnsureAboveZero;
	for (n = options.getElementsByTagName("select"),
	i = n.length - 1; i >= 0; --i)
			(t = n[i]).onchange = t.onclick = t.onkeypress = editorUpdateFollower
}
...
function editorUpdateFollower(e) {
	if (editor.in_settings)
			return editorUpdateSettingsOption(e);
	var t, n = editor.current_thing;
	(t = editor.follower) && (t.id = "",
	killNormal(t)),
	t = n.createfunc ? n.createfunc(editor.current_thing, editorGetArguments()) : ThingCreate(window[editor.current_thing_name], n.followerUpdate(editor.current_thing, editorGetArguments())),
	editor.follower = t,
	proliferate(t, {
			id: "follower",
	...
	addThing(t),
	addClass(t, "editor"),
	thingRetrieveVelocity(t),
	thingStoreVelocity(t),
	editorSetFollowerPosition(t),
	editor.erasing && (t.hidden = !0)
}
function editorGetArguments() {
	var e = arrayMake(editor.options.getElementsByTagName("input"))
		...
		, n = e.concat(t);
	return pairs = generateInputNameValuePairs(n),
	pairs
}
...
function editorSetFollowerPosition(e, t, n) {
	t = t || editor.xloc_old || 0,
	n = n || editor.yloc_old || 0,
	setLeft(e, t),
	setTop(e, n),
	editor.xloc_old = t,
	editor.yloc_old = n
...
function editorFollowerUpdateStandard(e, t) {
	...
	var n = [];
	return t.width && n.push(Number(t.width)),
	t.height && n.push(Number(t.height)),
	n
}
function editorClickControl(e) {
	...
	var t = e.target;
	t.id || (t = t.parentNode),
	window["editorControl" + capitalizeFirst(t.id)](),
	e.preventDefault()
}
...
function editorGetRawFunc() {
	var e, t = editor.placed, n = t.length - 1, x = new Array(e), r = "  var map = arguments[0] || new Map();\n";
	...
	e = n; e >= 0; --e)
			x[e] = new editorPreStatement(t[e]);
	for (x.sort(prethingsorter),
	e = n; e >= 0; --e)
			x[e] = "      " + x[e].statement;
...
function editorPreStatement(e) {
	this.placer = e,
	this.xloc = (gamescreen.left + e.left) / unitsize,
	this.yloc = map.floor - e.top / unitsize,
	this.statement = editorGetStatement(this, e, e.reference, e.args)
}
function editorGetStatement(e, t, n, x) {
	if (!n && !(n = editor[t.libtype][t.title]))
			return "";
	var r, i, a = (n.prefunc || pushPreThing).name, o = x.length;
	if (n.prefunc_custom)
			a += "(" + n.prefunc_custom(e, t, n, x) + ");";
	else {
			r = [],
			n.prefunc_solo || r.push(t.title),
			r.push(String(e.xloc)),
			r.push(String(e.yloc));
			for (var l = 1; l < o; ++l) {
					switch (typeof (i = x[l])) {
					case "undefined":
							break;
...
function Eraser(e) {
	e.width = e.height = 2,
	e.nocollide = e.nofall = !0,
	e.movement = eraserErases,
	setCharacter(e, "eraser")
}
function eraserErases(e) {
	if (window.editor) {
			var t, n, x = editor.placed, r = x.concat(solids).concat(characters).concat(scenery);
...
function addThingsToPlaced() {
	var e = editor.placed;
	for (editor.placed = (editor.placed || []).concat(characters).concat(solids).concat(scenery),
	e.sort(prethingsorter),
	e.splice(e.indexOf(player), 1),
	i = e.length - 1; i >= 0; --i)
			placer = e[i],
...
function editorCloseInputWindow(e) {
	...
	e || (editorSetCurrentThingFromName(),
	window.editing = !0),
	editorUpdateFollower()
}
function editorClose(e) {
	if (window.editor) {
			...
			for (t in n)
					removeChildSafe(document.getElementById(n[t]), body);
			document.onmousemove = null,
			window.editing = !1,
			e && window.map && (map.shifting = !1)
	}
}
function scrollEditor(e, t) {
	window.editor && (editor.follower && (e = e || 0,
	t = t || 0,
	shiftAll(scenery, e, t),
	shiftAll(solids, e, t),
	shiftAll(characters, e, t),
	editor.xloc += e,
	editor.yloc += t))
}
...
function setEditorLocalRetrieval() {
	localStorage.editorLastFunc && (editor.rawfunc = round,
...
function editorSubmitGameFunc() {
	...
	window.canedit = !0,
	setMap(["Custom", "Map"]),
	window.canedit = editor.playing = !1,
	entryBlank(player),
	addThingsToPlaced(),
	editorStoreLocally(),
...
function editorStartPlaying() {
	...
	for (n in x)
			thingRetrieveVelocity(e = x[n]),
			(t = editor[e.libtype][e.title]) && proliferate(e, t.onadds);
...
function pushRandomSectionOverworld(e) {
	var t = max(randTrue(117), 1)
		, n = 0;
	if (++map.num_random_sections,
	pushPreFuncCollider(e, zoneDisableCheeps),
	map.had_floor = !1,
	map.needs_floor || t >= 14 || t < 3 || randTrue() ? (pushPreFloor(e, 0, t),
	map.had_floor = !0) : pushPreThing(Stone, e, n = 0, t),
	...
	t <= 3 && map.had_floor) {
			...
			for (var x = t - 2, r = 0, i = 0, a = randTrue(2); a < x; a += 3)
					randTrue(7) && (randTrue(2) ? (map.hadObstacle = !1,
					(i % 3 == 0 || randTrue()) && (pushRandomChunkEnemy(e, a),
					++i),
					map.had_floor && pushRandomGroundScenery(e + 8 * a, a, t)) : pushRandomObstacle(e, a),
					!r && randTrue() ? (pushRandomSkyScenery(e + 8 * a),
					r = !0) : r = !1);
	prepareNextGeneratorStandard(e, t, pushRandomSectionOverworld)
}
function startRandomSectionBridge(e) {
	pushPreFuncCollider(e - 24, zoneDisableCheeps),
	...
	pushPreTree(e, 0, t + 1),
	pushPreThing(Stone, e + 16, 8, 1, 1),
	pushPreThing(Stone, e + 24, 16, 1, 2),
	pushPreThing(Stone, e + 32, 24, n, 3),
	pushRandomSectionBridge(e + 8 * (t - 1), 24, !0),
	spawnMap(),
	map.had_floor = !1
}
function pushRandomSectionBridge(e, t, n) {
	var x = !1;
	if (t = t || 24 + 16 * randTrue() - 8,
	randTrue() || map.needs_bridge)
			switch (randTrue(3)) {
			case 0:
					switch (randTrue()) {
					case 0:
							var r = randTrue(3) + 1
								, i = 4 * r;
							x = !0;
							for (var a = 1; a <= 2 * r; a += 2)
									pushPreBridge(e + 16 * a, t, 3);
							break;
					case 1:
							i = randTrue(7) + 7;
							var o = DtB(t, 8);
							n || pushPreThing(Stone, e, t, 1, o),
							pushPreBridge(e + 8, t, i - 1),
			...
			default:
					...
					i = 17 * (randTrue(3) + 3);
					for (a = 0; a < i; a += 17)
							if (!a && n || pushPreThing(Stone, e + 8 * a, t, 1, o),
							pushPreBridge(e + 8 * (a + 1), t, 16),
							randTrue() && pushRandomSmallEnemy(e + 8 * (a + 8), t),
							randTrue(2)) {
									randTrue();
									var l = 3 + randTrue(2);
									pushRandomCoinRow(e + 8 * (a + 8), t + 32, l),
									pushRandomCoinRow(e + 8 * (a + 8), t + 40, getNextCoinRowSize(l))
							} else
									pushPreThing(Block, e + 8 * (a + 8), t + jumplev1, Mushroom);
					pushPreThing(Stone, e + 8 * i, t, 1, o)
			}
	else
			i = 10,
			pushPreTree(e + 16, 8 * randTrue(), i);
...
function pushRandomSectionPreCastle(e, t) {
	var n, x, r = randTrue(35) + 35, i = r - 3, a = !1;
	for (t = t || 0,
	pushPreFloor(e, 0, r),
	hadcloud = !1,
	x = randTrue(3); x < i; x += n || 3) {
			switch (randTrue(3)) {
			case 0:
					switch (n = 3,
					randTrue(2)) {
					...
					case 1:
							for (var l = 0; l < n; ++l)
									randTrue() || pushPreThing(Stone, e + 8 * (x + l), 8 * (o = randTrue(3) + 1), 1, o);
							break;
					case 2:
			...
			case 1:
					...
					case 2:
							...
							a = !1;
							for (l = 1; l < 8; ++l)
									for (var s = jumplev1; s <= jumplev2; s += 32)
											pushPreThing(Brick, e + 8 * (x + l), s, getRandomBrickItem(!1, randTrue()));
							var d = randTrue() ? jumplev1 : jumplev2;
							height2 = o == jumplev1 ? jumplev2 : jumplev1,
							randTrue(2) && pushPreThing(HammerBro, e + 8 * (x + randTrue(3)), d + 12),
							randTrue(2) && pushPreThing(HammerBro, e + 8 * (x + 4 + randTrue(3)), height2 + 12)
					}
			}
			a && n >= 7 && (pushPreScenery("CastleWall", e + 8 * (x + randTrue()), 0, n - randTrue(2)),
			randTrue() && pushPreThing(Brick, e + 8 * (x + randTrue(n)), jumplev1, randTrue() ? Mushroom : getRandomBrickItem(!1, randTrue())));
			for (s = 0; s < n; s += 3)
					randTrue(2) && pushRandomGroundScenery(e + 8 * (x + s), 0),
					!hadcloud && randTrue() ? (pushRandomSkyScenery(e + 8 * (x + s)),
					hadcloud = !0) : hadcloud = !1
	}
	pushPreFloor(e + 8 * r, 0, x + 3 - r);
	var u = 4 + randTrue(3);
	t >= 3 ? endCastleOutsideRandom(e + 8 * (r + u + 1), !0) : pushRandomSectionPreCastle(e + 8 * (r + u), t + 1),
	spawnMap()
}
function endCastleOutsideRandom(e) {
	...
	switch (randTrue()) {
	case 0:
			for (n = 1 + randTrue(); n < 9; n += 2)
					pushPreThing(Stone, e + 8 * n, 8 * (n - randTrue()), 1, 1 + randTrue());
			pushPreThing(Stone, e + 72, 64, 2),
			t = 12;
			break;
	case 1:
			for (pushPreFloor(e, 0, 11),
			n = 1,
			hadlast = !1; n < 9; ++n)
					!hadlast || randTrue(2) || 8 == n ? (pushPreThing(Stone, e + 8 * n, 8 * n, 1, n),
					hadlast = !0) : (hadlast = !1,
					pushPrePipe(e + 8 * n, 0, 8 * max(n - randTrue(2), 2), !0),
					++n);
			pushPreThing(Stone, e + 72, 72, 2, 9),
			t = 7
	}
	pushPreFloor(e + 88, 0, round(gamescreen.width / 8)),
	endCastleOutside(e + 8 * (9 + t) + 4, 0, !0, round(gamescreen.width / 8))
}
function startRandomSectionCastle(e) {
	...
	randTrue(4);
	pushPreFloor(e, 24, t),
	pushPreThing(Stone, e, 88, t, 3),
	fillPreWater(e + 8 * t, 0, 2 * n),
	pushPreThing(Podoboo, e + 8 * t + max(0, 8 * randTrue(n - 3)), -32),
	pushRandomSectionCastle(e + 8 * (t + n), 0),
	spawnMap()
}
function pushRandomSectionCastle(e, t) {
	...
	case 0:
			...
			for (var r = 0; r < n; ++r)
					randTrue() ? (makeCeilingCastle(e + 64 * r, 8),
					fillPreWater(e + 64 * r, 0, 16),
					pushPreThing(Platform, e + 64 * r + 8 + 8 * randTrue(2), 8 + 8 * randTrue(max(r + 2, 4)), 4, moveFalling)) : (pushPreFloor(e + 64 * r - 8, 8, 1),
					pushPrePlatformGenerator(e + 64 * r + 24, 4, 1.75),
					pushPreFloor(e + 64 * r + 64, 8, 1));
			break;
	case 1:
			var i, a, o;
			x = 64 * (n = 2 * (1 + randTrue())) - 8,
			makeCeilingCastle(e, 8 * n),
			fillPreWater(e, 0, 16 * n);
			for (r = 0; r < n; ++r) {
					switch (i = e + 64 * r,
					a = 8 * randTrue(max(r + 1, 2 + randTrue(2))),
					o = 2 + randTrue(3),
					randTrue(2)) {
					case 0:
					...
					randTrue(2) || o % 2 != 1 || pushPreThing(Block, i + 4 * o - 8, a + 40, Mushroom),
					o <= 4 && pushPreThing(Podoboo, i + 8 * (o + 1), -32)
			}
			break;
	case 2:
			var l;
			switch (x = 8 * (n = 14 + randTrue(21)) - 8,
			p = 1,
			pushPreFloor(e, 0, n),
			randTrue()) {
			case 0:
					makeCeilingCastle(e, n, p = 3);
					for (r = 1 + randTrue(); r < n - 6; ++r)
							l = min(7, n - r),
							pushPreThing(Stone, e + 8 * r, jumplev1, l),
							randTrue() && pushPreThing(CastleBlock, e + 8 * (r + l - 4), 0, [6, randTrue()], !0),
							pushPreThing(CastleBlock, e + 8 * (r + l), jumplev1, 6, randTrue()),
							randTrue() && pushPreThing(CastleBlock, e + 8 * (r + l + 4), jumplev2 + 8, [6, randTrue()], !0),
							r += l;
					break;
			case 1:
					...
					for (r = randTrue(2); r < n - 3; r += 4)
							pushPreThing(Stone, e + 8 * (r + d), 16 + 8 * s, 3, 2),
							pushPreThing(CastleBlock, e + 8 * (r + d + 1), 24 + 8 * s, randTrue(2) ? 6 : 0, randSign()),
							pushPreThing(Stone, e + 8 * (r + d), 80, 3, 2),
							r < n - 5 && pushPreThing(CastleBlock, e + 8 * (r + d + 1), 64, randTrue(2) ? 6 : 0, randSign()),
							r += 1 + randTrue(3)
			}
			break;
	case 3:
			...
			var u = 1 + randTrue(3)
				, p = 11 - u - 4;
			pushPreFloor(e, 8 * u, n),
			makeCeilingCastle(e, n, p);
			for (r = 0; r < n; r += 8)
					randTrue() && (pushRandomEnemy(e + 8 * r, 8 * u, 0),
					randTrue() && (pushRandomEnemy(e + 8 * r + 12, 8 * u, 0),
					randTrue() && pushRandomEnemy(e + 8 * r + 24, 8 * u, 0)))
	}
	pushPreThing(GenerationStarter, e + x, ceilmax + 20, t <= 280 ? pushRandomSectionCastle : endCastleInsideRandom, t + x / 8),
	spawnMap()
}
function endCastleInsideRandom(e) {
	...
	for (var r = 0; r < t; ++r)
			pushPreFloor(e + 5 * (r + 1) * 8, x, max(2, randTrue(3)));
	var i = e + 5 * t * 8
		, a = 8 * randTrue(7);
	fillPreWater(i, 0, a),
	endCastleInsideRandomFinal(i + a),
	spawnMap()
}
function endCastleInsideRandomFinal(e) {
	fillPreWater(e, 0, 16),
	pushPreFloor(e + 24, 24, 3),
	endCastleInside(e + 48, 2),
	randTrue() && pushPreThing(Podoboo, e + 72 + 8 * randTrue(3), -32),
	randTrue() && fillPreThing(Brick, e + 56 + 8 * randTrue(3), 64, 3 + randTrue(3), 1, 8),
	randTrue() && pushPreThing(CastleBlock, e + 56 + 8 * randTrue(2), 24, [6, randSign()], !0),
	spawnMap()
}
...
function pushRandomSectionTrees(e) {
	...
	case 0:
			...
			t = randTrue(14) + 7,
			n = randTrue(3),
			map.treefunc(e, 8 * n, t);
			for (var a = randTrue(2); a < t - 2; a += r - 1)
					randTrue(2) && pushRandomSmallEnemy(e + 8 * a, 8 * n),
					randTrue(2) && (r = 3 + (randTrue(3) ? 0 : randTrue(4)),
					i = 8 * min(9, n + randTrue(7) + 3),
					map.treefunc(e + 8 * a, i, r),
					randTrue() && pushRandomSmallEnemy(e + 8 * a, 8 * i),
					pushRandomSmallEnemy(e + 8 * (a += r - 1), 8 * n));
			break;
	case 1:
			t = 14,
			n = 7,
			randTrue(),
			t = 4 + randTrue(2),
			pushPrePlatformGenerator(e + 8 * (randTrue() + 1), t, -1),
			t += randTrue(3) + 3;
			break;
	default:
			t = 4 + randSign() + randTrue(),
			n = min(randTrue(2) + 4 + randSign(2), map.treelev + 4);
			var o = e - 8 * randTrue();
			if (n == map.treelev && (n += randSign()),
			map.treefunc(o, 8 * n, t),
			t > 3 || randTrue())
					if (randTrue(3))
							pushRandomSmallEnemy(o + 8 * (randTrue() + 1), 8 * n);
					else if (randTrue(2))
							for (a = 1; a < t - 1; ++a)
									pushPreThing(Coin, o + 1 + 8 * a, 8 * (n + 1) - 1)
	}
	++map.sincechange > 7 && randTrue() ? (x = map.randtype,
	map.sincechange = 0) : x = pushRandomSectionTrees,
	pushPreThing(GenerationStarter, e + 8 * (t + randSign()), ceilmax + 20, x),
	spawnMap(),
	map.treelev = n
}
...
function pushRandomSectionUnderworld(e) {
	var t, n, x = max(randTrue(117), 1), r = 14, i = x - x % r;
	floor(x / r);
	if (pushPreFloor(e, 0, x),
	window.randcount_powerup = 3,
	x < r)
			switch (randTrue()) {
			case 0:
					for (t = 0; t < x - 2; t += 3)
							pushRandomChunkEnemy(e + 8 * t, 0, t);
					break;
			case 1:
					for (t = 0; t < x - 2; t += 3)
							randTrue(2) ? t % 3 == 0 && pushRandomChunkEnemy(e, t) : pushRandomObstacle(e, t)
			}
	else
			for (t = 1; t < i; t += r)
					switch (randTrue(5)) {
					case 0:
							pushRandomUnderworldSquigglies(e + 8 * t, r),
							makeCeiling(e + 8 * t, r);
							break;
					case 1:
							var a = 1 + randTrue()
								, o = 4 + randTrue(7);
							for (t += a,
							n = 0; n < r; n += 1 + randTrue() / 2)
									randTrue() && pushRandomSmallEnemy(e + 8 * (t + n), 0);
							fillPreThing(Brick, e + 8 * t, 8 * o, 13, 12 - o, 8, 8),
							t -= a;
							break;
					case 2:
							...
							break;
					case 3:
							pushUnderworldPipes(e + 8 * (t + 2), 12),
							makeCeiling(e + 8 * (t + 1), r);
							break;
					case 4:
							pushUnderworldStones(e + 8 * (t + 2), 12),
							makeCeiling(e + 8 * (t + 1), r);
							break;
					case 5:
							for (n = 0; n < 10; n += 3)
									pushRandomChunkEnemy(e + 8 * (t + n), n);
							break;
					case 6:
							for (n = 0; n < x - 2; n += 3)
									randTrue(2) ? (t % 3 == 0 || randTrue()) && (pushRandomChunkEnemy(e + 8 * (t + n), n),
									++numenemychunks) : pushRandomObstacle(e + 8 * (t + n), n)
					}
	prepareNextGeneratorStandard(e, x, pushRandomSectionUnderworld, !0),
	spawnMap()
}
function pushRandomUnderworldSquigglies(e, t, n, x) {
	...
	for (var r = 3 + randTrue(2), i = min(n, r + 1 + randTrue(4)), a = 1 == r ? i : randTrue(2) ? r : i, o = 1 + i - r, l = !1, s = i + 16, d = 0; d < t; ++d) {
			if (randTrue()) {
					for (var u = 0; u < 3; ++u)
							pushPreThing(Brick, e + 8 * (d + u), 8 * a, randTrue() ? null : getRandomBrickItem());
					!x && randTrue(2) && fillPreThing(Coin, e + 1 + 8 * d, 8 * min(s, a + randTrue(4) + 1) - 1, 3 + randTrue(), 1, 8),
					l || (randTrue() ? pushPreThing(Block, e + 8 * (d + 3), 8 * a, getRandomBlockItem()) : pushPreThing(Brick, e + 8 * (d + 3), 8 * a),
					l = !0),
					d += 3
			} else
					fillPreThing(Brick, e + 8 * d, 8 * r, 1, o, 8, 8),
					a = a == i ? r : i,
					l = !1;
...
function pushUnderworldPipes(e, t) {
	...
	for (r = 0; r < i; r += 4)
			switch (randTrue()) {
			case 0:
					addPipeRandom(e + 8 * (r + (n = randTrue())), 0, 8 * (2 + randTrue(2))),
					r += 1 - n,
					a = !0;
					break;
			case 1:
					n = randTrue() || !a,
					x = 4 + randTrue(4),
					n && pushPreThing(Brick, e + 8 * r, 8 * max(x - 4, 3 + randTrue()), getRandomBrickItem(!1)),
					addPipeRandom(e + 8 * (r + n), 0, 8 * x),
					a = !1
			}
	for (; r < t - 1; ++r)
			randTrue() && pushRandomChunkEnemy(e + 8 * r, 0)
}
function pushUnderworldStones(e, t) {
	...
	for (r = 0; r < i; r += 2)
			switch (randTrue()) {
			case 0:
					pushPreThing(Stone, e + 8 * (r + (n = randTrue())), 8 * (x = 2 + randTrue(2)), 1, x),
					r += 1 - n,
					a = !0;
					break;
			case 1:
					...
					x = 4 + randTrue(4),
					n && pushPreThing(Brick, e + 8 * r, 8 * max(x - 4, 3 + randTrue()), getRandomBrickItem(!1)),
					pushPreThing(Stone, e + 8 * (r + n), 8 * x, 1, x),
					a = !1
			}
	for (; r < t - 1; r += 3)
			randTrue() && pushRandomChunkEnemy(e + 8 * r, 0)
...
function pushRandomSectionUnderwater(e) {
	...
	pushPreFloor(e, 0, t -= t % 3),
	pushPreScenery("Water", e, ceilmax - 21, 8 * t / 3, 1),
	pushPreThing(WaterBlock, e, ceilmax, 8 * t),
	window.randcount_powerup = 3;
	for (var n = 0; n < t; n += 4) {
			switch (randTrue(21)) {
			case 0:
			...
			default:
					...
					case 0:
							var x = randTrue() + 2
								, r = randTrue() + 2;
							pushPreThing(Stone, e + 8 * n, 8 * r, randTrue(3) + 1, r),
							pushPreThing(Stone, e + 8 * n, ceillev, randTrue(3) + 1, x);
							break;
					case 1:
							randTrue() && pushPreThing(Stone, e + 8 * n, jumplev1, 4),
							randTrue() && pushPreThing(Stone, e + 8 * n, jumplev2, 4);
							break;
					case 2:
					...
					default:
							...
							map.had_coral = !0;
							var i, a = randTrue(3) + 2, o = jumplev1 * (1 + randTrue(2)), l = e + 8 * n;
							if (o == 3 * jumplev1) {
									var s = !0;
									o -= 8
							}
							pushPreThing(Stone, e + 8 * n, o, a),
							i = !s && (randTrue(3) || a <= 3) ? o + 24 : o - 8,
							randTrue() && pushPreThing(Coral, l, i, 3),
							randTrue() && a > 3 && o < 64 && pushPreThing(Coral, l + 8 * (a - 1), i, 3),
							a >= 3 && (n += a - 3)
					}
			}
			map.countCheep > 1 && (pushPreThing(CheepCheep, e + 8 * n, randTrue(80) + 8, randTrue()),
			map.countCheep = 0),
			map.countBlooper > 7 && (pushPreThing(Blooper, e + 8 * n, randTrue(80) + 8),
			map.countBlooper = 0),
			randTrue(7) && ++map.countCheep,
			randTrue(3) && ++map.countBlooper
	}
	if (++map.sincechange < 3) {
			var d = prepareNextGeneratorStandard(e, t, pushRandomSectionUnderwater, !1, !0);
			pushPreScenery("Water", e + 8 * t, ceilmax - 21, 8 * (d + 1) / 3, 1),
			pushPreThing(WaterBlock, e + 8 * t, ceilmax, 8 * (d + 1))
	} else
			endRandomSectionUnderwater(e + 8 * t)
}
function endRandomSectionUnderwater(e) {
	pushPreFloor(e, 0, 19),
	pushPreScenery("Water", e, ceilmax - 21, 28, 1),
	pushPreThing(WaterBlock, e, ceilmax, 157.5),
	pushPreThing(Stone, e, 8, 5, 1),
	pushPreThing(Stone, e + 8, 16, 4, 1),
	pushPreThing(Stone, e + 16, 24, 3, 1),
	pushPreThing(Stone, e + 24, 32, 2, 1),
	pushPreThing(Stone, e + 24, 88, 2, 4),
	pushPreThing(PipeSide, e + 32, 48, ["Random", randTrue() ? "Overworld" : "Underworld", "Up"]),
	pushPreThing(Stone, e + 40, 88, 14, 11),
	map.scrollblockerok = !0,
	pushPreThing(ScrollBlocker, e + 56, 80, !0),
	spawnMap()
}
function startRandomSectionSky(e) {
	pushPreThing(Stone, e, 0, 78),
	pushPreThing(Platform, e + 88, 24, 6, [collideTransport]),
	pushRandomSectionSky(e + 80, 1),
	spawnMap()
}
function pushRandomSectionSky(e, t) {
	if (t++ > 7)
			return fillPreThing(Coin, e + 8, 8, 3, 1, 8),
			spawnMap();
	var n = 0;
	if (t % 2)
			fillPreThing(Coin, e + 1, 71, 3, 1, 8),
			n = 32;
	else
			switch (randTrue(t)) {
			case 3:
					pushPreThing(Stone, e + 8, 48, 1, 2),
					fillPreThing(Coin, e + 25, 63, 7, 1, 8),
					pushPreThing(Stone, e + 88, 48, 1, 2),
					n = 104;
					break;
			case 4:
			case 5:
			case 6:
					...
					for (var x = 0; x <= 7; x += 2)
							pushPreThing(Stone, e + 8 * (x + 5), 56),
							fillPreThing(Coin, e + 8 * (x + 5) + 1, 63, 2, 1, 8);
					n = 104;
					break;
			default:
					fillPreThing(Coin, e + 1, 55 + 8 * randTrue(), 16, 1, 8),
					n = 128
			}
	pushPreThing(GenerationStarter, e + n, ceilmax + 20, pushRandomSectionSky, t),
	spawnMap()
}
function prepareNextGeneratorStandard(e, t, n, x, r) {
	...
	if (r)
			...
			switch (randTrue(7)) {
			case 0:
					t > 7 && map.underwater && !randTrue(7) ? (i = randTrue(3) + 7,
					pushPreThing(Springboard, e + 8 * (t - 1), 14.5)) : a = !0;
					break;
			case 1:
					var o = max(1, randTrue(7));
					i = o + randTrue(3),
					pushPreFloor(e + 8 * t, 0, o);
					for (var l = 1; l <= o; ++l)
							pushPreThing(Stone, e + 8 * (t + l - 1), 8 * l, 1, l);
					if (randTrue()) {
							pushPreFloor(e + 8 * (t + i + (o = max(1, randTrue(o))) - 1), 0, o);
							for (var s = 0; s < o; ++s)
									pushPreThing(Stone, e + 8 * (t + i + o + s - 1), 8 * (o - s), 1, o - s);
							i += o + o - 2
					}
					break;
			default:
	...
	return (a || !i || i < 1) && (i = randTrue(3) + 1),
	n == pushRandomSectionOverworld && map.num_random_sections >= 3 + randTrue(7) && (n = pushRandomSectionPreCastle),
	!r && ++map.sincechange > 3 && (n = getRandomNextSection(),
	map.sincechange = 0,
	pushPreFuncCollider(e, zoneDisableCheeps)),
	pushPreThing(GenerationStarter, e + 8 * (t + i), ceilmax + 20, n),
	spawnMap(),
	i
}
...
function pushRandomChunkEnemy(e, t, n) {
	pushRandomEnemy(e, 0, t, n),
	randTrue(2) && (pushRandomSolidRow(e + 8 * t, jumplev1, randTrue(2) + 1),
	randTrue() && pushRandomEnemy(e, jumplev1, t + 1, !0),
	randTrue() && (pushRandomSolidRow(e + 8 * t, jumplev2, randTrue(2) + 1),
	randTrue() && pushRandomEnemy(e, jumplev2, t + 1, !0)))
}
function pushRandomEnemy(e, t, n, x) {
	...
	case 3:
			if (!x) {
					switch (randTrue(4)) {
					case 0:
							pushPreThing(HammerBro, e + 8 * n, t + 12),
							randTrue() && pushPreThing(HammerBro, e + 8 * n + 16, t + 40);
							break;
					case 1:
...
function addPipeRandom(e, t, n) {
	var x;
	x = !(n <= 24 || randTrue(2)) && getRandomTransport(),
	pushPrePipe(e, t, n, randTrue(7), x)
}
function getRandomTransport() {
	var e, t = [["Overworld", "Up"], ["Underworld", "Down"], ["Underwater", "Up"]];
	return t[e = randTrue(t.length - 1)][0] == map.randname && (e = (e + randTrue(t.length - 2) + 1) % t.length),
	["Random", t[e][0], t[e][1]]
}
...
function pushRandomObstacle(e, t) {
	var n = randTrue(3);
	switch (n > 1 && (map.hadPipe = !1),
	n) {
	case 0:
	case 1:
			if (t > 1) {
					addPipeRandom(e + 8 * t, 0, 8 * (randTrue(2 + (1 == map.hadObstacle && 0 == map.hadPipe && t > 7)) + 2)),
					map.hadObstacle = map.hadPipe = !0;
					break
			}
	case 2:
			for (var x = 0; x < 2; ++x)
					randTrue() || t < 1 || pushPreThing(Stone, e + 8 * (t + x), 8 * (r = randTrue(2) + 2), 1, r);
			break;
	default:
			x = randTrue(2);
			switch (randTrue(7)) {
			case 0:
					var r;
					if (pushPreThing(Cannon, e + 8 * (t + x), 8 * (r = randTrue(2) + 1), r),
					1 == r && randTrue(2) && 2 != x) {
							var i = randTrue() + 2;
			...
			case 1:
					if (!map.underwater) {
							randTrue() && (!map.underwater && randTrue(2) && pushPreThing(Brick, e + 8 * t, jumplev1),
							pushPreThing(Block, e + 8 * (t + 1), jumplev1, [Mushroom, 1], !0),
							!map.underwater && randTrue(2) && pushPreThing(Brick, e + 8 * (t + 2), jumplev1),
							map.hadObstacle = !0);
							break
					}
			case 2:
					if (!map.underwater) {
							var a = randTrue();
							a || pushPreThing(Brick, e + 8 * t, jumplev1, getRandomBrickItem()),
							pushPreThing(Stone, e + 8 * (t + a), jumplev1, 2),
							addPipeRandom(e + 8 * (t + a), jumplev1, 24 + 8 * randTrue()),
							a && pushPreThing(Brick, e + 8 * t, jumplev1, getRandomBrickItem());
							break
					}
			}
...
function pushRandomSolidRow(e, t, n) {
	for (var x = 0; x < n; ++x)
			randTrue(2) ? pushPreThing(Brick, e + 8 * x, t, getRandomBrickItem("Overworld" == map.randname && t == jumplev2)) : pushPreThing(Block, e + 8 * x, t, getRandomBlockItem())
}
...
function getRandomBlockItem() {
	return ++randcount_powerup,
	!(randcount_powerup <= 7) && (!randTrue(7) && Mushroom)
}
...
function addDistanceCounter() {
	...
	TimeHandler.addEventInterval((function(e) {
			...
			e.innerText = data.traveledold + data.traveled + " blocks traveled"
	}
	), 3, 1 / 0, counter)
}
function addSeedDisplay() {}
function createTunnel(e, t, n) {
	for (var x = randTrue(2) + 3, r = randTrue(2) + 2, i = !1, a = 0; a < t; ++a)
			fillPreThing(n, e + 8 * a, 8, 1, r, 8, 8),
			randTrue(3) || i ? i = !1 : pushRandomSmallEnemy(e + 8 * a, 8 * r),
			fillPreThing(n, e + 8 * a, 96 - 8 * x, 1, x, 8, 8)
}
function removeRandomDisplays() {
	var e, t = body.getElementsByClassName("randomdisplay");
	for (e = t.length - 1; e >= 0; --e)
			body.removeChild(t[e])
}
...
function applyPaletteFilter(e, t) {
	var n, x, r, i = "";
	for (x = 0,
	r = e.length; x < r; x += digitsize)
			i += t[n = e.substr(x, digitsize)] || n;
	return i
}
function evaluatePostMultiple(e) {
	var t, n, x = e[1], r = e[2], i = new SpriteMultiple(x);
	for (n in r)
			t = r[n],
			i[n] = "string" == typeof t ? spriteGetArray(spriteExpand(spriteUnravel(t))) : t;
	return i
}
...
function passivelyLoadMap(e, t) {
	if (!(!e || e[0] > 8 || e[1] <= 0)) {
			...
			t.open("GET", n, !0),
			mlog("Maps", "Requesting:", n),
			t.send(),
			t.onreadystatechange = function() {
					if (4 == t.readyState) {
							if (200 == t.status)
									mapfuncs[e[0]][e[1]] = Function(t.responseText),
									window.parentwindow && parentwindow.onMapLoad && (parentwindow.onMapLoad(e[0], e[1]),
									setTimeout((function() {
											parentwindow.onMapLoad(e[0], e[1])
									}
									), 2100)),
									mlog("Maps", " Loaded: Maps/World" + e[0] + e[1] + ".js");
...
function Map() {
	...
	this.maxyvel = 1.75 * unitsize,
	this.maxyvelinv = -2.1 * this.maxyvel
}
function Area(e, t) {
	this.creation = t || function() {}
	,
	this.precharacters = [],
	this.presolids = [],
	this.prescenery = [],
	...
	this.width = 0,
	this.underwater = !1,
	setAreaSetting(this, e || "")
}
function setAreaSetting(e, t, n) {
	map.shifting = !0,
	1 == arguments.length && (t = arguments[0] || "Overworld",
	e = map.area),
	e.setting = e.background = t,
	e.theme = t.split(" ")[0],
	e.fillStyle = getAreaFillStyle(e.setting),
	-1 != e.fillStyle.indexOf("Underwater") ? goUnderWater() : goOntoLand(),
	n && AudioPlayer.playTheme(),
	gameon && clearAllSprites(),
	map.shifting = !1
}
function Location(e, t, n) {
	...
	this.entry = 1 == t ? entryPlain : t || entryNormal
}
...
function setMap(e, t) {
	if (gameon) {
			!window.canedit && window.editing && editorClose(!0),
			removeRandomDisplays(),
			e instanceof Array && (t = e[1],
			e = e[0]);
			...
			window.currentmap = n,
			r(x),
			x.areanum = x.curloc = 0,
			window.area = x.area = x.areas[0],
			window.player && player.power && storePlayerStats(),
			window.data && (data.scoreold = data.score.amount),
			shiftToLocation(0)) : log("No such map exists (yet?):", r)) : log("No such map section exists (yet?):", r)
	}
}
function setMapRandom(e) {
	gameon && (resetSeed(),
	"string" == typeof e ? e = ["Random", e] : e || (e = ["Random", "Overworld"]),
	setMap(e[0], e[1]),
	data.traveledold = data.traveled,
	map.sincechange = map.num_random_sections = 0,
	map.entrancetype = e[2],
	map.random = !0,
	"Sky" == map.randname && (map.exitloc = ["Random", "Overworld", "Down"]))
}
function shiftToLocation(e) {
	if (map.random && "number" != typeof e)
			return setMapRandom(e);
	"number" == typeof e && (e = map.locs[e]),
	pause(),
	resetGameState(),
...
function setAreaPreCreation(e) {
	...
	setDataDisplay(),
	startDataTime(),
	map.random && (data.world.amount = "Random Map",
	data.world.element.innerHTML = "WORLD<br>Random Map")
}
function clearTexts() {
	if (window.texts)
			for (var e = texts.length - 1; e >= 0; --e)
					texts[e] && removeChildSafe(texts[e], body);
	window.texts = []
}
function setAreaPostCreation() {
	if (map.current_character = map.current_solid = map.current_scenery = 0,
	...
	area.underwater && (area.presolids.push(new PreThing(0,0,WaterBlock,area.width)),
	map.random || area.presolids.push(new PreThing(0,16,Sprite,"Water",[area.width / 3, 1]))),
	area.presolids.sort(prethingsorter),
	area.precharacters.sort(prethingsorter),
	area.prescenery.sort(prethingsorter),
	area.sections && area.sections[0])
			setBStretch(),
...
function spawnMap() {
	var e, t, n, x, r, i = map.area, a = QuadsKeeper.getOutDifference(), o = gamescreen.right + a, l = o + (2 * QuadsKeeper.getQuadWidth() + a);
	for (t = (e = i.precharacters).length,
	r = map.current_character; t > r && o >= (n = e[r]).xloc * unitsize; )
			addThing(x = n.object, n.xloc * unitsize - gamescreen.left, n.yloc * unitsize),
			x.placenum = r,
			++r;
	for (map.current_character = r,
	t = (e = i.presolids).length,
	r = map.current_solid; t > r && l >= (n = e[r]).xloc * unitsize; )
			addThing(x = n.object, n.xloc * unitsize - gamescreen.left, n.yloc * unitsize),
			x.placenum = r,
			++r;
	for (map.current_solid = r,
	t = (e = i.prescenery).length,
	r = map.current_scenery; t > r && l >= (n = e[r]).xloc * unitsize; )
			addThing(x = n.object, n.xloc * unitsize - gamescreen.left, n.yloc * unitsize),
			x.placenum = r,
			++r;
...
function goToTransport(e) {
	e instanceof Array ? (map.ending = !0,
	storePlayerStats(),
	pause(),
	map.random ? setMapRandom(e) : setMap(e)) : shiftToLocation(map.locs[e]),
	is_mobile && applyNewJs()
}
function entryPlain(e) {
	setLeft(e, unitsizet16),
	setBottom(e, map.floor * unitsize),
	e.nocollide = e.piping = !1,
	e.placed = !0
}
function entryNormal(e) {
	setLeft(e, unitsizet16),
	setTop(e, unitsizet16),
	e.nocollide = e.piping = !1,
	e.placed = !0
}
function entryBlank(e) {
	setLeft(e, unitsizet16),
	setBottom(e, map.floor * unitsize),
	e.nocollide = e.piping = e.movement = !1,
	e.placed = e.nofall = e.nocollide = notime = nokeys = !0,
	thingStoreVelocity(e),
	clearDataDisplay()
}
function entryRandom(e) {
	switch (data.time.amount = 0,
	data.time.dir = 1,
	updateDataElement(data.time),
	map.startwidth ? map.nofloor || pushPreFloor(0, 0, map.startwidth) : map.startwidth = 0,
	map.firstRandomThings(map),
	map.randtype(8 * (map.startwidth + 1)),
	entryPlain(e),
	addDistanceCounter(),
	addSeedDisplay(),
	map.entrancetype) {
...
function enterCloudWorld(e) {
	...
	e.placed = e.nofall = !0,
	setTop(e, t),
	setLeft(e, 30 * unitsize),
	removeClass(e, "jumping"),
	addClasses(e, ["climbing", "animated"]),
	e.climbing = TimeHandler.addSpriteCycle(e, ["one", "two"], "climbing"),
	e.attached = new Thing(Vine,-1),
	addThing(e.attached, unitsizet32, t - unitsizet8);
	var x = setInterval((function() {
			if (e.attached.top <= n) {
					clearInterval(x),
					setTop(e.attached, n, !0),
					e.attached.movement = !1;
					var t = e.attached.top + unitsizet16;
					x = setInterval((function() {
							shiftVert(e, -1 * unitsized4, !0),
							e.top <= t && (removeClass(e, "animated"),
							clearInterval(x),
							setTop(e, t, !0),
							clearInterval(x),
							setTimeout((function() {
									setLeft(e, 36 * unitsize, !0),
									addClass(e, "flipped"),
									setTimeout((function() {
											playerHopsOff(e, e.attached, !0),
											TimeHandler.clearClassCycle(e, "climbing"),
...
function walkToPipe() {
	...
	var e = setInterval((function() {
			player.piping && (AudioPlayer.pauseTheme(),
			clearInterval(e),
...
function intoPipeVert(e, t, n) {
	if (!(!t.transport || !e.resting || e.right + unitsizet2 > t.right || e.left - unitsizet2 < t.left)) {
			pipePreparations(e),
			switchContainers(e, characters, scenery),
			unpause();
			var x = setInterval((function() {
					shiftVert(e, unitsized4, !0),
					e.top >= t.top && (clearInterval(x),
					setTimeout((function() {
							goToTransport(n)
					}
					), 700))
...
function intoPipeHoriz(e, t, n) {
	...
	unpause();
	var x = setInterval((function() {
			shiftHoriz(e, unitsized4, !0),
			e.left >= t.left && (clearInterval(x),
			setTimeout((function() {
					goToTransport(n)
			}
			), 700))
...
function pipePreparations(e) {
	...
	locMovePreparations(e),
	e.nofall = e.nocollide = nokeys = notime = !0,
	e.movement = e.xvel = e.yvel = 0
}
function locMovePreparations(e) {
	...
	e.placed = !1,
	removeCrouch(),
	removeClass(e, "running"),
	removeClass(e, "jumping"),
	removeClass(e, "flipped")
}
function startCastle(e) {
	(e = e || window.player) && (setBottom(e, 56 * unitsize),
	setLeft(e, unitsizet2),
	e.nocollide = e.piping = !1,
	e.placed = !0)
}
function exitPipeVert(e, t) {
	switchContainers(e, characters, scenery),
	e.nofall = nokeys = notime = !0,
	AudioPlayer.play("Pipe"),
	setTop(e, t.top),
	setMidXObj(e, t, !0);
	var n = unitsize / -4
		, x = setInterval((function() {
			shiftVert(e, n, !0),
			e.bottom <= t.top && (switchContainers(e, scenery, characters),
			clearInterval(x),
			e.nocollide = e.piping = e.nofall = nokeys = notime = !1,
			e.placed = !0)
	}
	), timer)
}
...
function pushPreThing(e, t, n, x, r) {
	var i = new PreThing(map.refx + t,map.refy - n,e,x,r)
		, a = i.object;
	return !a.solid && !a.character || a.nostretch || (map.area.width = max(map.area.width, i.xloc + a.width)),
	a.solid && !a.spawn_as_char ? map.area.presolids.push(i) : map.area.precharacters.push(i),
	i
}
function pushPreScenery(e, t, n, x, r) {
	...
	return i.yloc -= i.object.height,
	map.area.prescenery.push(i),
	i
}
...
function makeCeiling(e, t) {
	t = t || 1;
	for (var n = 0; n < t; ++n)
			pushPreThing(Brick, e + 8 * n, ceillev)
}
function makeCeilingCastle(e, t, n) {
	pushPreThing(Stone, e, ceillev, t || 1, n || 1)
}
function pushPreBridge(e, t, n, x) {
	pushPreScenery("Railing", e, t, 2 * n),
	pushPreThing(BridgeBase, e, t, n),
	x instanceof Array && (x[0] && pushPreThing(Stone, e - 8, t, 1, 64),
	x[1] && pushPreThing(Stone, e + 8 * n, t, 1, 64))
}
function fillPreWater(e, t, n) {
	var x = DtB(t)
		, r = ceil(x / 5.5)
		, i = 5 * r;
	pushPreScenery("Water", e, t - 5.5, 4 * n / 3),
	pushPreScenery("WaterFill", e, t - i - 15.5, 4 * n / 3, r + 2)
}
...
function pushPreScale(e, t, n, x) {
	...
	platleft = pushPreThing(Platform, e - i, t - 4 * a, r, moveFallingScale).object,
	platright = pushPreThing(Platform, e + 4 * n - r - 6, t - 4 * o, r, moveFallingScale).object,
	platleft.parent = l,
	platright.parent = l,
	platleft.partner = platright,
	platright.partner = platleft,
	platleft.tension = a * unitsizet4 - 10 * unitsize,
	platright.tension = o * unitsizet4 - 10 * unitsize,
	l.tensionleft = a * unitsize,
	l.tensionright = o * unitsize,
	platleft.string = pushPreScenery("String", e, t - 4 * a, 1, 4 * (a - .5)).object,
	platright.string = pushPreScenery("String", e + 4 * n - 1, t - 4 * o, 1, 4 * (o - .5)).object
}
function pushPreWarpWorld(e, t, n, x, r) {
	1 == n.length && (n = [-1, n[0], -1]);
	var i, a, o = (x || 0) + e + 10, l = n.length;
	...
	for (warp.texts.push(s.object),
	a = 0; a < l; ++a)
			-1 != n[a] && (warp.pipes.push(i = pushPrePipe(o, t, 24, !0, n[a]).object),
			warp.pirhanas.push(i.pirhana),
			n[a]instanceof Array && warp.texts.push(pushPreText({
					innerText: n[a][0],
					style: {
							visibility: "hidden"
					}
			...
			o += 32;
	r && (window.block = pushPreThing(ScrollBlocker, e, ceilmax),
	pushPreThing(ScrollBlocker, o + 16, ceilmax))
}
function goUnderWater() {
	window.map && (map.area && (window.player && !map.shifting && setAreaSetting(String(map.area.setting || "") + " Underwater"),
	map.area.underwater = !0),
	setMapGravity(),
	TimeHandler.clearEvent(map.bubbling),
	map.bubbling = TimeHandler.addEventInterval(playerBubbles, 96, 1 / 0),
	map.underwater = !0)
}
function goOntoLand() {
...
function endCastleOutside(e, t, n, x, r) {
	...
	i.flag = pushPreThing(Flag, e + .5, t + 79.5).object,
	i.stone = pushPreThing(Stone, e + 4, t + 8).object,
	i.top = pushPreThing(FlagTop, e + 6.5, 84).object,
	i.pole = pushPreThing(FlagPole, e + 8, 80).object,
	x && pushPreScenery("CastleWall", e + r + 72, t, x),
	0 == n && shiftHoriz(a, unitsizet8),
	pushPreCastle(e + r + 16, t, n)
}
...
function endCastleInside(e, t, n) {
	...
	x.bridge = pushPreThing(CastleBridge, e, 24, 13).object,
	x.chain = pushPreThing(CastleChain, e + 96.5, 32).object,
	x.bowser = pushPreThing(Bowser, e + 69, 42, n).object,
	pushPreThing(ScrollBlocker, e + 112, ceilmax),
	pushPreThing(Stone, e, 88, 32),
	fillPreWater(e, 0, 26),
	pushPreFloor(e + 104, 32, 3),
	pushPreFloor(e + 104, 0, 19),
	pushPreThing(Stone, e + 112, 80, 2, 3),
	pushPreThing(ScrollBlocker, e + 256, ceilmax),
	endCastleInsideFinal(e, t)
}
function endCastleInsideFinal(e, t) {
	...
	t ? (pushPreThing(Peach, e + 194, 13).object,
	n.text = [pushPreText({
			...
			style: x
	}, e + 148, 26).object]) : (pushPreThing(Toad, e + 194, 12).object,
	n.text = [pushPreText({
			innerHTML: "<span class='span'>THANK YOU " + window.player.title.toUpperCase() + "!</span>",
			style: x
	}, e + 160, 66).object, pushPreText({
...
function pushPreSectionPass(e, t, n, x, r) {
	var i = pushPreThing(Collider, e, t, [n, x], [sectionPass, sectionColliderInit]).object
		, a = (r = map.area.sections.current || 0,
	map.area.sections[r]);
	a.numpass ? ++a.numpass : a.numpass = 1,
...
function sectionColliderInit(e) {
	e.sections = map.area.sections,
	e.parent = e.sections[e.sections.current],
	e.movement = !1
}
function sectionPass(e, t) {
	if ("player" != e.type)
			return !1;
	t.nocollide = !0,
	--t.parent.numpass || activateSection(t.parent, !0)
}
function sectionFail(e, t) {
	if ("player" != e.type)
			return !1;
	t.nocollide = !0,
	activateSection(t.parent, !1)
}
function activateSection(e, t) {
	for (var n = e.colliders, x = n.length - 1; x >= 0; --x)
			killNormal(n[x]);
	e.activated = !0,
	e.passed = t
}
...
function pushPrePipe(e, t, n, x, r, i) {
	isFinite(n) || (n = gamescreen.height,
	t -= gamescreen.height);
	var a = pushPreThing(Pipe, e, t + n, n / 8, r)
		, o = a.object;
	return x && (o.pirhana = pushPreThing(Pirhana, e + 4, t + n + 12).object),
	i && (map.locs[i].entrything = o,
	map.locs[i].xloc = e),
	a
}
function pushPreCastle(e, t, n) {
	e = e || 0,
	t = t || 0,
	n ? pushPreCastleBig(e, t) : pushPreCastleSmall(e, t)
}
function pushPreCastleBig(e, t) {
	var n, x;
	for (pushPreCastleSmall(e + 16, t + 48),
	n = 0; n < 3; ++n)
			for (x = 0; x < 2; ++x)
					pushPreScenerySolid("BrickPlain", e + 16 + 16 * n, t + 24 + 8 * x);
	for (n = 0; n < 2; ++n)
			pushPreScenerySolid("CastleDoor", e + 24 + 16 * n, t + 24);
	for (n = 0; n < 5; ++n) {
			if (2 == n)
					continue;
	...
	for (n = 0; n < 2; ++n)
			pushPreScenerySolid("CastleRailing", e + 8 * n, t + 44);
	for (n = 0; n < 5; ++n)
			pushPreScenerySolid("CastleRailingFilled", e + 16 + 8 * n, t + 44);
	for (n = 5; n < 7; ++n)
			pushPreScenerySolid("CastleRailing", e + 16 + 8 * n, t + 44);
	for (n = 0; n < 2; ++n)
			for (x = 0; x < 3; ++x)
					pushPreScenerySolid("BrickPlain", e + 24 + 16 * n, t + 8 * x);
	for (n = 0; n < 3; ++n)
			pushPreScenerySolid("CastleDoor", e + 16 + 16 * n, t);
	for (n = 0; n < 2; ++n) {
			for (x = 0; x < 5; ++x)
					pushPreScenerySolid("BrickPlain", e + 8 * n, t + 8 * x);
			pushPreScenerySolid("BrickHalf", e + 8 * n, t + 40)
	}
	for (n = 0; n < 2; ++n) {
			for (x = 0; x < 5; ++x)
					pushPreScenerySolid("BrickPlain", e + 56 + 8 * n, t + 8 * x);
			pushPreScenerySolid("BrickHalf", e + 56 + 8 * n, t + 40)
	}
	for (n = 0; n < 3; ++n)
			for (x = 0; x < 2; ++x)
					pushPreScenerySolid("BrickHalf", e + 16 + 16 * n, t + 20 + 20 * x)
}
function pushPreCastleSmall(e, t) {
	var n, x;
	for (n = 0; n < 3; ++n)
			pushPreScenerySolid("CastleRailing", e + 8 + 8 * n, t + 36);
	for (n = 0; n < 2; ++n)
			pushPreScenerySolid("CastleTop", e + 8 + 12 * n, t + 24);
	for (pushPreScenerySolid("CastleRailing", e, t + 20),
	n = 1; n <= 3; ++n)
			pushPreScenerySolid("CastleRailingFilled", e + 8 * n, t + 20);
	for (pushPreScenerySolid("CastleRailing", e + 32, t + 20),
	n = 0; n < 2; ++n)
			for (pushPreScenerySolid("BrickHalf", e + 8 * n, t),
			x = 0; x < 2; ++x)
					pushPreScenerySolid("BrickPlain", e + 8 * n, t + 4 + 8 * x);
	for (n = 0; n < 2; ++n)
			for (pushPreScenerySolid("BrickHalf", e + 24 + 8 * n, t),
			x = 0; x < 2; ++x)
					pushPreScenerySolid("BrickPlain", e + 24 + 8 * n, t + 4 + 8 * x);
	pushPreScenerySolid("CastleDoor", e + 16, t)
}
function pushPreFuncCollider(e, t) {
	return e instanceof Array ? (console.log("position", e),
	pushPreThing(FuncCollider, e[0], e[1], t, [e[2], e[3]])) : pushPreThing(FuncCollider, e, ceilmax + 40, t)
}
...
function zoneDisableLakitu() {
	if (map.has_lakitu) {
			...
			map.zone_lakitu = map.has_lakitu = !1,
			e.lookleft || (e.lookleft = !0,
			removeClass(e, "flipped")),
...
function pushPrePattern(e, t, n, x) {
	var r, i, a, o = t, l = Scenery.patterns[e];
	for (i = 0; i < x; ++i) {
			for (a in l)
					(r = l[a])instanceof Array && pushPreScenery(r[0], o + r[1], n + r[2], r[3], r[4]);
			o += l.width
	}
}
function DtB(e, t) {
...
function BlankMap(e) {
	e.locs = [new Location(0,entryBlank)],
...
function randMapType(e) {
	...
	e.areas = [new Area(e.areatype,(function() {
			setLocationGeneration(0),
			"Underwater" == e.randname && (goUnderWater(),
			pushPreScenery("Water", 0, ceilmax - 21, 8 * (e.startwidth + 1) / 3, 1),
			pushPreThing(WaterBlock, 0, ceilmax, 8 * (e.startwidth + 1)))
	}
	))],
	e.treefunc = randTrue(3) ? pushPreTree : pushPreShroom,
	e.treeheight = e.treelev = e.sincechange = 0
}
...
function WorldRandomOverworld(e) {
	e.random = !0,
	e.randtype = pushRandomSectionOverworld,
	e.randname = "Overworld",
	e.areatype = "Overworld" + randDayNight(),
	e.firstRandomThings = function(e) {
			for (var t = 0; t < 10; ++t)
					randTrue() && pushRandomGroundScenery(8 * t)
	}
	,
	e.startwidth = 14,
	e.onlysmartkoopas = !1,
...
function WorldRandomTrees(e) {
	e.random = !0,
	e.randtype = pushRandomSectionTrees,
	e.randname = "Overworld",
	e.areatype = "Overworld" + randDayNight(),
	e.firstRandomThings = function(e) {
			e.treefunc(100, 8 * (e.treelev = randTrue() + 2), randTrue() + 4),
			e.startwidth += 7
	}
	,
	e.startwidth = 11,
	e.onlysmartkoopas = randTrue(),
	randMapType(e)
}
function WorldRandomUnderworld(e) {
	e.random = !0,
	e.randtype = pushRandomSectionUnderworld,
	e.randname = e.areatype = "Underworld",
	e.firstRandomThings = function(e) {
			fillPreThing(Brick, 0, 8, 1, 11, 8, 8)
	}
	,
	e.startwidth = randTrue(3) + 7,
	e.onlysmartkoopas = !0,
	e.respawndist = 42,
	e.entrancetype = "Up",
	randMapType(e)
...
function WorldRandomUnderwater(e) {
	e.random = !0,
	e.randtype = pushRandomSectionUnderwater,
	e.randname = "Underwater",
	e.areatype = "Underwater" + randDayNight(),
	e.firstRandomThings = function(e) {}
	,
	e.startwidth = randTrue(3) + 7,
	e.entrancetype = "Up",
	e.countCheep = e.countBlooper = 0,
	e.respawndist = 42,
	e.onlysmartkoopas = !0,
	randMapType(e)
}
function WorldRandomBridge(e) {
	e.random = !0,
	e.randtype = startRandomSectionBridge,
	e.randname = "Overworld",
	e.areatype = "Overworld" + randDayNight(),
	e.firstRandomThings = function(e) {}
	,
...
function WorldRandomSky(e) {
	e.random = !0,
	e.randtype = startRandomSectionSky,
	e.randname = "Sky",
	e.areatype = "Sky" + randDayNight(),
	e.entrancetype = "Vine",
	e.firstRandomThings = function(e) {
...
function WorldRandomCastle(e) {
	e.random = !0,
	e.randtype = startRandomSectionCastle,
	e.randname = e.areatype = e.entrancetype = "Castle",
	e.firstRandomThings = function(e) {
			startCastleInside(),
			startCastle()
	}
...
function FullScreenMario() {
	...
	window.Uint8ClampedArray = window.Uint8ClampedArray || window.Uint8Array || Array,
	resetMeasurements(),
	resetLibrary(),
	resetEvents(),
	resetCanvas(),
	...
	resetSeed(),
	resetSounds(),
	window.luigi = localStorage && "true" == localStorage.luigi,
	window.gameon = !0,
	setMap(1, 1),
	log("It took " + (Date.now() - e) + " milliseconds to start.")
}
function ensureLocalStorage() {
	...
	if (!e)
			throw document.body.innerText = "It seems your browser does not allow localStorage!"
}
function resetMeasurements() {
	...
	window.paused = !0,
	resetGameScreen(),
	window.parentwindow || (window.parentwindow = !1)
}
function resetUnitsize(e) {
	window.unitsize = e;
	for (var t = 2; t <= 64; ++t)
			window["unitsizet" + t] = unitsize * t,
			window["unitsized" + t] = unitsize / t;
	window.scale = unitsized2,
	window.gravity = round(12 * unitsize) / 100
}
function resetTimer(e) {
	e = roundDigit(e, .001),
	window.timer = window.timernorm = e,
	window.timert2 = 2 * e,
	window.timerd2 = e / 2,
	window.fps = window.fps_target = roundDigit(1e3 / e, .001),
...
function getGameScreen() {
	...
	var e = 2;
	is_mobile && (e = 3),
	this.middlex = (this.left + this.right) / e,
	window.botmax = this.height - ceilmax,
	botmax < unitsize && (body.innerHTML = "<div><br>Your screen isn't high enough. Make it taller, then refresh.</div>"),
	this.deathheight = this.bottom + 48
}
function resetGameScreenPosition(e) {
	(e = e || window.gamescreen).left = e.top = 0,
	e.bottom = innerHeight,
	e.right = innerWidth,
	e.height = innerHeight / unitsize,
	e.width = innerWidth / unitsize,
	e.unitheight = innerHeight,
	e.unitwidth = innerWidth
}
...
function resetGameState(e) {
	clearAllTimeouts(),
	resetData(),
	window.nokeys = window.spawning = window.spawnon = window.notime = window.editing = window.qcount = window.lastscroll = 0,
	window.paused = window.gameon = window.speed = 1,
	e || (window.gamecount = 0),
	resetQuadrants(),
	window.gamehistory = [],
	window.sumLeftScroll = 0,
	window.firstMiddle = !1,
	AudioPlayer.pause()
}
function scrollWindow(e, t) {
	...
	gamescreen.top += t,
	gamescreen.bottom += t,
	shiftAll(characters, n, x),
	shiftAll(solids, n, x),
	shiftAll(scenery, n, x),
	shiftAll(QuadsKeeper.getQuadrants(), n, x),
	shiftElements(texts, n, x),
	QuadsKeeper.updateQuadrants(n),
	window.playediting && scrollEditor(e, t)
}
...
function shiftElements(e, t, n) {
	for (var x, r = e.length - 1; r >= 0; --r)
			x = e[r],
			elementShiftLeft(x, t),
			elementShiftTop(x, n)
}
function scrollPlayer(e, t, n) {
	var x = player.left
		, r = player.top;
	scrollWindow(e, t = t || 0),
	setLeft(player, x, n),
	setTop(player, r + t * unitsize, n),
...
function resetCanvas() {
	window.canvas = getCanvas(innerWidth, innerHeight, !0),
	window.context = canvas.getContext("2d"),
	body.appendChild(canvas)
}
function spriteUnravel(e) {
	for (var t, n, x, r = getPaletteReferenceStarting(window.palette), i = window.digitsize, a = e.length, o = "", l = 0; l < a; )
			switch (e[l]) {
			case "x":
					for (x = e.indexOf(",", ++l),
					t = makeDigit(r[e.slice(l, l += i)], window.digitsize),
					n = Number(e.slice(l, x)); n--; )
							o += t;
					l = x + 1;
					break;
			case "p":
					"[" == e[++l] ? (x = e.indexOf("]"),
					r = getPaletteReference(e.slice(l + 1, x).split(",")),
					l = x + 1,
					i = 1) : (r = getPaletteReference(window.palette),
					i = window.digitsize);
					break;
			default:
...
function spriteExpand(e) {
	for (var t, n, x = "", r = e.length, i = 0; i < r; )
			for (t = e.slice(i, i += digitsize),
			n = 0; n < scale; ++n)
					x += t;
	return x
}
function spriteGetArray(e) {
	var t, n, x, r, i = e.length / digitsize, a = e.match(new RegExp(".{1," + digitsize + "}","g")), o = new Uint8ClampedArray(4 * i);
	for (n = 0,
	x = 0; n < i; ++n) {
			for (t = palette[Number(a[n])],
			r = 0; r < 4; ++r)
					o[x + r] = t[r];
			x += 4
	}
...
function setThingSprite(e) {
	if (!e.hidden && e.title) {
			var t, n = library.cache, x = e.spritewidth, r = e.spriteheight;
			n[e.title + " " + e.className.split(/\s+/g).slice(1).sort()];
			(t = getSpriteFromLibrary(e)) ? t.multiple ? (expandObtainedSpriteMultiple(t, e, x, r),
			e.sprite_type = t.type) : (expandObtainedSprite(t, e, x, r),
			e.sprite_type = "normal") : log("Could not get sprite from library on " + e.title)
	}
}
function getSpriteFromLibrary(e) {
	var t, n, x, r, i = library.cache, a = e.title, o = e.libtype, l = e.className.split(/\s+/g).slice(1).sort(), s = (map.area || window.defaultsetting).setting.split(" ");
	for (r in s)
			l.unshift(s[r]);
	if (n = i[t = a + " " + l])
			x = n.raw;
	else {
			if (!(x = library.sprites[o][a]) || !x.constructor)
					return console.log("Error in checking for sprite of " + a + "."),
					void console.log("Title " + a, "\nLibtype " + o, "\n", e, "\n");
			x.constructor != Uint8ClampedArray && (x = findSpriteInLibrary(e, x, l)),
			n = i[t] = {
					raw: x
			}
	}
...
function expandObtainedSprite(e, t, n, x, r) {
	var i, a, o = new Uint8ClampedArray(e.length * scale), l = n * unitsizet4, s = x * scale, d = 0, u = 0;
	for (i = 0; i < s; ++i) {
			for (a = 0; a < scale; ++a)
					memcpyU8(e, o, d, u, l),
					u += l;
...
function expandObtainedSpriteMultiple(e, t, n, x) {
	var r, i, a = {};
	for (i in t.num_sprites = 0,
	e)
			(r = e[i]).constructor == Uint8ClampedArray ? (++t.num_sprites,
			a[i] = expandObtainedSprite(r, t, n, x, !0)) : a[i] = "number" == typeof r ? r * scale : r;
	t.sprite = a.middle,
	t.sprites = a,
	refillThingCanvases(t, a)
}
function findSpriteInLibrary(e, t, n) {
	...
	if (t.multiple)
			return t;
	for (var o = 0; x = !0; ) {
			for (i in ++o > 49 && (alert(e.title),
			console.log(e.title, n, t)),
			n)
					if (r = t[n[i]]) {
							t = r,
							n.splice(i, 1),
			...
			if (x)
					if (r = t.normal)
							...
							x = !0;
			if (x || !t)
					return console.log("\nSprite not found! Title: " + e.title),
					console.log("Classname:", e.className),
					console.log("Remaining", n),
					console.log("Current", t),
					console.log("Prev", a),
					new Uint8ClampedArray(e.spritewidth * e.spriteheight);
...
function refillThingCanvas(e) {
	...
	memcpyU8(e.sprite, x.data),
	n.putImageData(x, 0, 0)
}
function refillThingCanvases(e, t) {
	var n, x, r, i, a, o = e.canvases = {}, l = e.spritewidthpixels, s = e.spriteheightpixels;
	for (a in e.num_sprites = 1,
	t)
			(n = t[a])instanceof Uint8ClampedArray ? (++e.num_sprites,
			o[a] = r = {
					canvas: getCanvas(l, s)
			},
			r.context = i = r.canvas.getContext("2d"),
			memcpyU8(n, (x = i.getImageData(0, 0, l, s)).data),
			i.putImageData(x, 0, 0)) : o[a] = n;
	r = o.middle,
	e.canvas = r.canvas,
	e.context = r.context
}
function refillCanvas() {
	var e, t = window.canvas, n = window.context;
	for (n.fillStyle = area.fillStyle,
	n.fillRect(0, 0, t.width, t.height),
	e = scenery.length - 1; e >= 0; --e)
			drawThingOnCanvas(n, scenery[e]);
	for (e = solids.length - 1; e >= 0; --e)
			drawThingOnCanvas(n, solids[e]);
	for (e = characters.length - 1; e >= 0; --e)
			drawThingOnCanvas(n, characters[e])
}
...
function drawThingOnCanvasMultiple(e, t, n, x, r, i) {
	var a, o, l = i, s = r, d = x.right, u = x.bottom, p = x.unitwidth, c = x.unitheight, m = x.spritewidthpixels, h = x.spriteheightpixels;
	"v" == x.sprite_type[0] ? ((o = t.bottom) && (a = t.bottomheight || x.spriteheightpixels,
	drawPatternOnCanvas(e, o.canvas, s, u - a, m, min(c, h)),
	u -= a,
	c -= a),
	(o = t.top) && (a = t.topheight || x.spriteheightpixels,
	drawPatternOnCanvas(e, o.canvas, s, l, m, min(c, h)),
	l += a,
	c -= a)) : "h" == x.sprite_type[0] && ((o = t.left) && (a = t.leftwidth || x.spritewidthpixels,
	drawPatternOnCanvas(e, o.canvas, s, l, min(p, m), h),
	s += a,
	p -= a),
	(o = t.right) && (a = t.rightwidth || x.spritewidthpixels,
	drawPatternOnCanvas(e, o.canvas, d - a, l, min(p, m), h),
	d -= a,
	p -= a)),
	l < u && s < d && drawPatternOnCanvas(e, n, s, l, p, c)
}
function getPaletteReferenceStarting(e) {
	for (var t = {}, n = 0; n < e.length; ++n)
			t[makeDigit(n, digitsize)] = makeDigit(n, digitsize);
	return t
}
function getPaletteReference(e) {
	for (var t = {}, n = getDigitSize(e), x = 0; x < e.length; ++x)
			t[makeDigit(x, n)] = makeDigit(e[x], n);
	return t
}
function flipSpriteArrayHoriz(e, t) {
	var n, x, r, i, a, o = e.length, l = t.spritewidth, s = (t.spriteheight,
	new Uint8ClampedArray(o)), d = l * unitsizet4;
	for (r = 0; r < o; r += d)
			for (n = r,
			x = r + d - 4,
			i = 0; i < d; i += 4) {
...
function flipSpriteArrayVert(e, t) {
	for (var n, x, r = e.length, i = t.spritewidth, a = (t.spriteheight,
	new Uint8ClampedArray(r)), o = i * unitsizet4, l = 0, s = r - o; l < r; ) {
			for (n = 0; n < o; n += 4)
					for (x = 0; x < 4; ++x)
							a[l + n + x] = e[s + n + x];
			l += o,
...
function drawPatternOnCanvas(e, t, n, x, r, i) {
	e.translate(n, x),
	e.fillStyle = e.createPattern(t, "repeat"),
	e.fillRect(0, 0, r, i),
	e.translate(-n, -x)
}
...
function memcpyU8(e, t, n, x, r) {
	if (!(!e || !t || n < 0 || x < 0 || r <= 0 || n >= e.length || x >= t.length)) {
			null == n && (n = 0),
			null == x && (x = 0),
			null == r && (r = max(0, min(e.length, t.length)));
			for (var i = r + 0, a = x + 0, o = n + 0; i--; )
					t[a++] = e[o++]
	}
}
function canvasDisableSmoothing(e, t) {
	(t = t || e.getContext("2d")).webkitImageSmoothingEnabled = !1,
...
function Thing(e) {
	if (0 != arguments.length && e) {
			...
			e.apply(t, n),
			t.alive = !0,
			t.placed = this.outerok = 0,
			t.xvel = this.xvel || 0,
			t.yvel = this.yvel || 0,
			null == t.tolx && (t.tolx = 0),
			null == t.toly && (t.toly = unitsized8),
			t.collide = t.collide || function() {}
			,
			t.death = t.death || killNormal,
			t.animate = t.animate || emergeUp;
			var x, r = 4;
			(x = floor(t.width * unitsize / QuadsKeeper.getQuadWidth())) > 0 && (r += (x + 1) * r / 2),
			(x = floor(t.height * unitsize / QuadsKeeper.getQuadHeight())) > 0 && (r += (x + 1) * r / 2),
			t.maxquads = r,
			t.quadrants = new Array(t.maxquads),
			t.overlaps = [],
			t.title = t.title || e.name,
			t.spritewidth = t.spritewidth || t.width,
			t.spriteheight = t.spriteheight || t.height,
			t.sprite = "";
			try {
					setContextStuff(t, t.spritewidth, t.spriteheight)
			} catch (e) {
					log("Thing context fail", e, t.title, t),
					setTimeout((function() {
							setContextStuff(t, t.spritewidth, t.spriteheight)
					}
					), 1)
...
function setContextStuff(e, t, n) {
	e.spritewidthpixels = e.spritewidth * unitsize,
	e.spriteheightpixels = e.spriteheight * unitsize,
	e.canvas = getCanvas(e.spritewidthpixels, e.spriteheightpixels),
	e.context = e.canvas.getContext("2d"),
	e.imageData = e.context.getImageData(0, 0, e.spritewidthpixels, e.spriteheightpixels),
	e.sprite_type = e.sprite_type || "neither",
	canvasDisableSmoothing(e, e.context)
}
function ThingCreate(e, t) {
	var n = new Thing;
	return Thing.apply(n, [e].concat(t)),
	n
}
function setCharacter(e, t) {
	e.type = t.split(" ")[0],
	e.resting = e.under = e.undermid = !1,
	e.alive = e.character = !0,
	e.libtype = "characters",
	setClassInitial(e, "character " + t)
}
function setSolid(e, t) {
	...
	e.solid = e.alive = !0,
	e.speed = e.speed || 0,
	e.collide = e.collide || characterTouchedSolid,
	e.bottomBump = e.bottomBump || function() {}
	,
	e.action = e.action || function() {}
	,
	e.jump = e.jump || function() {}
	,
	e.spritewidth = e.spritewidth || 8,
	e.spriteheight = e.spriteheight || 8,
	e.libtype = "solids",
	setClassInitial(e, "solid " + t)
}
...
function addThing(e, t, n) {
	return e instanceof Function && (e = new Thing(e)),
	placeThing(e, t, n),
	window[e.libtype].push(e),
	e.placed = !0,
	e.onadding && e.onadding(),
	setThingSprite(e),
	window["last_" + (e.title || e.group || "unknown")] = e,
	e
}
...
function addText(e, t, n) {
	...
	return body.appendChild(x),
	texts.push(x),
	x
}
function spawnText(e, t) {
	var n = e.element = addText("", e.left, e.top);
	"object" == typeof t ? proliferate(n, t) : n.innerHTML = t,
	e.movement = !1
}
function checkTexts() {
	var e, t, n, x = QuadsKeeper.getDelX();
	for (n = texts.length - 1; n >= 0; --n)
			t = texts[n],
			e = texts[n].element || t,
			t.right = t.left + e.clientWidth,
			t.right < x && (body.removeChild(e),
			killNormal(t),
			deleteThing(e, texts, n))
}
function Mushroom(e, t) {
	...
	e.speed = .42 * unitsize,
	e.animate = emergeUp,
	e.movement = moveSimple,
	e.collide = collideFriendly,
	e.jump = mushroomJump,
	e.death = killNormal,
	e.nofire = !0;
	var n = "mushroom";
...
function FireFlower(e) {
	...
	e.nofall = e.nofire = !0,
	e.movement = !1,
	setCharacter(e, "fireflower"),
	TimeHandler.addSpriteCycle(e, ["one", "two", "three", "four"])
}
function FireBall(e, t) {
	...
	e.nofire = e.nostar = e.collide_primary = !0,
	e.moveleft = t,
	e.animate = emergeFire,
	e.movement = moveJumping,
	e.collide = fireEnemy,
	e.death = fireExplodes,
	setCharacter(e, "fireball"),
	TimeHandler.addSpriteCycle(e, ["one", "two", "three", "four"], 4)
}
function fireEnemy(e, t) {
	if (!(!t.alive || t.emerging || e.height <= unitsize)) {
			if (e.nofire)
					return e.nofire > 1 ? t.death(t) : void 0;
			e.solid ? AudioPlayer.playLocal("Bump", t.right) : (AudioPlayer.playLocal("Kick", t.right),
			e.death(e, 2),
			scoreEnemyFire(e)),
			t.death(t)
	}
...
function fireExplodes(e) {
	...
	addThing(t, e.left - t.width / 2, e.top - t.height / 2),
	t.animate(),
	killNormal(e)
}
function Star(e) {
	...
	e.height = 8,
	e.speed = .56 * unitsize,
	e.jumpheight = 1.17 * unitsize,
	e.gravity = gravity / 2.8,
	e.animate = emergeUp,
	e.movement = moveJumping,
	e.collide = collideFriendly,
	e.action = playerStar,
	e.death = killNormal,
	e.nofire = !0,
	setCharacter(e, "star item"),
	TimeHandler.addSpriteCycle(e, ["one", "two", "three", "four"], 0, 7)
}
function Shell(e, t) {
	e.width = 8,
	e.height = 7,
	...
	e.speed = unitsizet2,
	e.collide_primary = !0,
	e.moveleft = e.xvel = e.move = e.hitcount = e.peeking = e.counting = e.landing = e.enemyhitcount = 0,
	e.smart = t,
	e.movement = moveShell,
	e.collide = hitShell,
	e.death = killFlip,
	e.spawntype = Koopa,
	setCharacter(e, "shell" + (t ? " smart" : " dumb"))
}
function hitShell(e, t) {
	if ("shell" == e.type && t.type != e.type)
			return hitShell(t, e);
	switch (e.type) {
	case "solid":
			t.right < e.right ? (AudioPlayer.playLocal("Bump", e.left),
			setRight(t, e.left),
			t.xvel = -t.speed,
			t.moveleft = !0) : (AudioPlayer.playLocal("Bump", e.right),
			setLeft(t, e.right),
			t.xvel = t.speed,
			t.moveleft = !1);
			break;
	case "player":
			...
			if (e.star)
					return scorePlayerShell(e, t),
					t.death(t, 2);
			if (t.landing)
					return void (t.shelltoleft == n ? (++t.landing,
					1 == t.landing && scorePlayerShell(e, t),
					TimeHandler.addEvent((function(e) {
							--e.landing
					}
					), 2, t)) : player.death(player));
			0 == t.xvel || x ? (t.counting = 0,
			scorePlayerShell(e, t),
			t.peeking && (t.peeking = !1,
			removeClass(t, "peeking"),
			t.height -= unitsized8,
			updateSize(t)),
			0 == t.xvel ? (n ? (t.moveleft = !0,
			t.xvel = -t.speed) : (t.moveleft = !1,
			t.xvel = t.speed),
			++t.hitcount,
			TimeHandler.addEvent((function(e) {
					--e.hitcount
			...
			x && (AudioPlayer.play("Kick"),
			t.xvel ? scorePlayerShell(e, t) : (jumpEnemy(e, t),
			e.yvel *= 2,
			scorePlayerShell(e, t),
			setBottom(e, t.top - unitsize, !0)),
			++t.landing,
			t.shelltoleft = n,
			TimeHandler.addEvent((function(e) {
					--e.landing
	...
	case "shell":
			if (0 != e.xvel)
					if (0 != t.xvel) {
							var r = e.xvel;
							shiftHoriz(e, e.xvel = t.xvel),
							shiftHoriz(t, t.xvel = r)
					} else
							score(t, 500),
							t.death(t);
			else
					0 != t.xvel && (score(e, 500),
					e.death(e));
			break;
	default:
			switch (e.group) {
			case "enemy":
					if (t.xvel) {
							if ("koopa" == e.type.split(" ")[0]) {
									...
									addThing(i, e.left, e.bottom - i.height * unitsize),
									killFlip(i),
									killNormal(e)
							} else
									killFlip(e);
							AudioPlayer.play("Kick"),
							score(e, findScore(t.enemyhitcount), !0),
							++t.enemyhitcount
					} else
							e.moveleft = objectToLeft(e, t);
					break;
			case "item":
					if ("shell" != e.type)
							return;
...
function moveShell(e) {
	if (0 == e.xvel)
			if (350 == ++e.counting)
					...
					addThing(t, e.left, e.bottom - t.height * unitsize),
					killNormal(e)
			}
}
function collideFriendly(e, t) {
	"player" == e.type && (t.action && t.action(e),
	t.death(t))
}
function jumpEnemy(e, t) {
	e.keys.up ? e.yvel = -1.4 * unitsize : e.yvel = -.7 * unitsize,
	e.xvel *= .91,
	AudioPlayer.play("Kick"),
	"item" == t.group && "shell" != t.type || score(t, findScore(e.jumpcount++ + e.jumpers), !0),
	++e.jumpers,
	TimeHandler.addEvent((function(e) {
			--e.jumpers
	}
...
function Goomba(e) {
	...
	e.speed = .21 * unitsize,
	e.toly = unitsize,
	e.moveleft = e.noflip = !0,
	e.smart = !1,
	e.group = "enemy",
	e.movement = moveSimple,
	e.collide = collideEnemy,
	e.death = killGoomba,
	setCharacter(e, "goomba"),
	TimeHandler.addSpriteCycleSynched(e, [unflipHoriz, flipHoriz])
}
function killGoomba(e, t) {
	if (e.alive)
			if (t)
					...
					addThing(n, e.left, e.bottom - n.height * unitsize),
					TimeHandler.addEvent(killNormal, 21, n),
					killNormal(e)
			}
}
function DeadGoomba(e) {
	...
	e.height = 4,
	e.movement = !1,
	e.nocollide = e.nocollide = !0,
	e.death = killNormal,
	setSolid(e, "deadGoomba")
}
function Koopa(e, t, n) {
	e.width = 8,
	e.height = 12,
	e.speed = e.xvel = .21 * unitsize,
	e.moveleft = e.skipoverlaps = !0,
	e.group = "enemy",
	e.smart = t;
	var x = "koopa";
	x += e.smart ? " smart" : " dumb",
	e.smart && (x += " smart"),
	n ? (x += " flying",
	e.winged = !0,
	1 == n ? (e.movement = moveJumping,
	e.jumpheight = 1.17 * unitsize,
	e.gravity = gravity / 2.8) : (e.movement = moveFloating,
	e.ytop = e.begin = n[0] * unitsize,
	e.ybot = e.end = n[1] * unitsize,
	e.nofall = e.fly = !0,
	e.changing = e.xvel = 0,
	e.yvel = e.maxvel = unitsized4)) : (x += " regular",
	e.smart ? e.movement = moveSmart : e.movement = moveSimple),
	e.collide = collideEnemy,
	e.death = killKoopa,
	setCharacter(e, x),
	TimeHandler.addSpriteCycleSynched(e, ["one", "two"]),
	e.toly = unitsizet2
}
function killKoopa(e, t) {
	if (e.alive) {
			var n;
			if (n = t && 2 != t || e.winged ? new Thing(Koopa,e.smart) : new Thing(Shell,e.smart),
			TimeHandler.addEvent((function(e, t) {
					addThing(e, t.left, t.bottom - e.height * unitsize),
					e.moveleft = t.moveleft
			}
			), 0, n, e),
			killNormal(e),
			2 != t)
...
function Pirhana(e, t) {
	...
	e.height = 12,
	e.counter = 0,
	e.countermax = e.height * unitsize,
	e.dir = unitsized8,
	e.toly = unitsizet8,
	e.nofall = e.deadly = e.nocollidesolid = e.repeat = !0,
	e.group = "enemy",
	e.collide = collideEnemy,
	e.death = killNormal,
	e.movement = movePirhanaInit,
	e.death = killPirhana,
	setCharacter(e, "pirhana")
}
function movePirhanaInit(e) {
	e.hidden = !0;
	var t = e.visual_scenery = new Thing(Sprite,"Pirhana");
	addThing(t, e.left, e.top),
	TimeHandler.addSpriteCycle(t, ["one", "two"]),
	e.movement = movePirhanaNew,
	movePirhanaNew(e, e.height * unitsize)
}
function movePirhanaNew(e, t) {
	t = t || e.dir,
	e.counter += t,
	shiftVert(e, t),
	shiftVert(e.visual_scenery, t),
	(e.counter <= 0 || e.counter >= e.countermax) && (e.movement = !1,
	e.dir *= -1,
	TimeHandler.addEvent(movePirhanaRestart, 35, e))
}
...
function killPirhana(e) {
	(e || (e = this)) && (killNormal(e),
	killNormal(e.visual_scenery))
}
...
function collideEnemy(e, t) {
	if (characterIsAlive(e) && characterIsAlive(t) && !(e.nocollidechar && !t.player || t.nocollidechar && !e.player)) {
			if ("item" == e.group)
					return e.collide_primary ? e.collide(t, e) : void 0;
			if (!map.underwater && e.player && (e.star && !t.nostar || !t.deadly && objectOnTop(e, t))) {
					if (playerAboveEnemy(e, t))
							return;
					...
					t.death(t, 2 * e.star);
					e.star ? scoreEnemyStar(t) : (scoreEnemyStomp(t),
					setBottom(e, min(e.bottom, t.top + unitsize))),
					addClass(e, "hopping"),
...
function Podoboo(e, t) {
	e.width = 7,
	e.height = 8,
	e.deadly = e.nofall = e.nocollidesolid = e.nofire = !0,
	e.gravity = map.gravity / 2.1,
	e.jumpheight = (t || 64) * unitsize,
	e.speed = -map.maxyvel,
	e.movement = movePodobooInit,
	e.collide = collideEnemy,
	e.betweentime = 70,
	setCharacter(e, "podoboo")
}
function movePodobooInit(e) {
	characterIsAlive(e) && (e.hidden = !0,
	e.heightnorm = e.top,
	e.heightfall = e.top - e.jumpheight,
	TimeHandler.addEvent(podobooJump, e.betweentime, e),
	e.movement = !1)
}
function podobooJump(e) {
	characterIsAlive(e) && (unflipVert(e),
	e.yvel = e.speed + e.gravity,
	e.movement = movePodobooUp,
	e.hidden = !1,
	setThingSprite(e))
}
function movePodobooUp(e) {
	shiftVert(e, e.speed, !0),
	e.top - gamescreen.top > e.heightfall || (e.nofall = !1,
	e.movement = movePodobooSwitch)
}
function movePodobooSwitch(e) {
	e.yvel <= 0 || (flipVert(e),
	e.movement = movePodobooDown)
}
function movePodobooDown(e) {
	e.top < e.heightnorm || (setTop(e, e.heightnorm, !0),
	e.movement = !1,
	e.nofall = e.hidden = !0,
	e.heightfall = e.top - e.jumpheight,
	TimeHandler.addEvent(podobooJump, e.betweentime, e))
...
function HammerBro(e) {
	...
	e.group = "enemy",
	e.collide = collideEnemy,
	e.statex = e.counter = e.statey = e.counterx = e.countery = e.level = e.throwcount = 0,
	e.death = killFlip,
	e.movement = moveHammerBro,
	setCharacter(e, "hammerbro"),
	e.gravity = gravity / 2,
	TimeHandler.addSpriteCycle(e, ["one", "two"]),
	TimeHandler.addEvent(throwHammer, 35, e, 7),
	TimeHandler.addEventInterval(jumpHammerBro, 140, 1 / 0, e)
}
function moveHammerBro(e) {
	e.xvel = Math.sin(Math.PI * (e.counter += .007)) / 2.1,
	lookTowardPlayer(e),
	e.nocollidesolid = e.yvel < 0 || e.falling
}
function throwHammer(e, t) {
	!characterIsAlive(e) || e.nothrow || e.right < -unitsizet32 || (3 != t && switchClass(e, "thrown", "throwing"),
	TimeHandler.addEvent((function(e) {
			if (3 != t) {
					if (!characterIsAlive(e))
							return;
					switchClass(e, "throwing", "thrown"),
					addThing(new Thing(Hammer,e.lookleft), e.left - unitsizet2, e.top - unitsizet2)
			}
			t > 0 ? TimeHandler.addEvent(throwHammer, 7, e, --t) : (TimeHandler.addEvent(throwHammer, 70, e, 7),
			removeClass(e, "thrown"))
	}
	), 14, e))
}
function jumpHammerBro(e) {
	if (!characterIsAlive(e))
			return !0;
	e.resting && (map.floor - e.bottom / unitsize >= jumplev1 - 2 && "floor" != e.resting.name && Math.floor(2 * Math.random()) ? (e.yvel = -.7 * unitsize,
	e.falling = !0,
	TimeHandler.addEvent((function(e) {
			e.falling = !1
	}
	), 42, e)) : e.yvel = -2.1 * unitsize,
	e.resting = !1)
}
function Hammer(e, t) {
	e.width = e.height = 8,
	e.nocollidesolid = e.nocollidechar = e.deadly = e.nofire = !0,
	e.collide = collideEnemy,
	e.yvel = 1.4 * -unitsize,
	e.xvel = unitsize / 1.4,
	t && (e.xvel *= -1),
	e.gravity = gravity / 2.1,
	setCharacter(e, "hammer"),
	TimeHandler.addSpriteCycle(e, ["one", "two", "three", "four"], 3)
}
function Cannon(e, t, n) {
	e.width = 8,
	e.height = 8 * (t || 1),
	e.spriteheight = 16,
	n || (e.movement = moveCannonInit),
	e.timer = 117,
	e.repeat = !0,
	setSolid(e, "cannon")
}
function moveCannonInit(e) {
	TimeHandler.addEventInterval((function(e) {
			if (!(player.right > e.left - unitsizet8 && player.left < e.right + unitsizet8)) {
					...
					objectToLeft(player, e) ? (addThing(t, e.left, e.top),
					t.direction = t.moveleft = !0,
					t.xvel *= -1,
					flipHoriz(t)) : addThing(t, e.left + e.width, e.top),
					AudioPlayer.playLocal("Bump", e.right)
			}
	}
	), 270, 1 / 0, e),
	e.movement = !1
...
function BulletBill(e) {
	...
	e.height = 7,
	e.group = "enemy",
	e.nofall = e.nofire = e.nocollidesolid = e.nocollidechar = !0,
	e.speed = e.xvel = unitsized2,
	e.movement = moveSimple,
	e.collide = collideEnemy,
	e.death = killFlip,
	setCharacter(e, "bulletbill")
}
function Bowser(e, t) {
	e.width = e.height = 16,
	e.speed = .28 * unitsize,
	e.gravity = gravity / 2.8,
	e.deadly = e.dx = e.lookleft = e.nokillend = e.skipoverlaps = !0,
	e.moveleft = e.smart = e.movecount = e.jumpcount = e.firecount = e.deathcount = 0,
	e.killonend = freezeBowser,
	e.counter = -.7,
	e.group = "enemy",
	e.movement = moveBowserInit,
	e.collide = collideEnemy,
	e.death = killBowser,
	setCharacter(e, "bowser"),
	TimeHandler.addSpriteCycle(e, ["one", "two"]),
	t && TimeHandler.addEvent(throwHammer, 35, e, 7)
}
function moveBowserInit(e) {
	TimeHandler.addEventInterval(bowserJumps, 117, 1 / 0, e),
	TimeHandler.addEventInterval(bowserFires, 280, 1 / 0, e),
	TimeHandler.addEventInterval(bowserFires, 350, 1 / 0, e),
	TimeHandler.addEventInterval(bowserFires, 490, 1 / 0, e),
	e.movement = moveBowser
}
function moveBowser(e) {
	characterIsAlive(player) && (lookTowardPlayer(e),
	e.lookleft ? e.xvel = Math.sin(Math.PI * (e.counter += .007)) / 1.4 : e.xvel = min(e.xvel + .07, .84))
}
function bowserJumps(e) {
	...
	TimeHandler.addEventInterval((function(e) {
			if (e.yvel > unitsize)
					return e.nocollidesolid = !1,
					!0
	}
	), 3, 1 / 0, e))
}
function bowserFires(e) {
	if (!characterIsAlive(e) || !characterIsAlive(player))
			return !0;
	e.lookleft && (addClass(e, "firing"),
	AudioPlayer.playLocal("Bowser Fires", e.left),
	TimeHandler.addEvent((function(e) {
			var t = e.top + unitsizet4
				, n = new Thing(BowserFire,roundDigit(player.bottom, unitsizet8));
			removeClass(e, "firing"),
			addThing(n, e.left - unitsizet8, t),
...
function killBowser(e, t) {
	if (t)
			return e.nofall = !1,
			killFlip(e);
	5 == ++e.deathcount && (e.yvel = e.speed = e.movement = 0,
	killFlip(e, 350),
	score(e, 5e3))
}
...
function BowserFire(e, t) {
	...
	e.collide = collideEnemy,
	t && (e.ylev = t,
	e.movement = moveFlying),
	setCharacter(e, "bowserfire"),
	TimeHandler.addSpriteCycle(e, [unflipVert, flipVert])
}
...
function WaterBlock(e, t) {
	e.height = 16,
	e.width = t,
	e.spritewidth = e.spriteheight = 1 / scale,
	e.repeat = !0,
	setSolid(e, "water-block")
}
function Blooper(e) {
	e.width = 8,
	e.height = 12,
	e.nocollidesolid = e.nofall = e.moveleft = 1,
	e.squeeze = e.counter = 0,
	e.speed = unitsized2,
	e.xvel = e.speedinv = -unitsized4,
	e.movement = moveBlooper,
	e.collide = collideEnemy,
	e.death = killFlip,
	setCharacter(e, "blooper")
}
function moveBlooper(e) {
	...
	e.top < unitsizet16 + 10 && squeezeBlooper(e),
	e.squeeze ? e.yvel = max(e.yvel + .021, .7) : e.yvel = min(e.yvel - .035, -.7),
	shiftVert(e, e.yvel, !0),
	e.squeeze || (player.left > e.right + unitsizet8 ? e.xvel = min(e.speed, e.xvel + unitsized32) : player.right < e.left - unitsizet8 && (e.xvel = max(e.speedinv, e.xvel - unitsized32)))
}
function squeezeBlooper(e) {
	2 != e.squeeze && addClass(e, "squeeze"),
	e.squeeze = 2,
	e.xvel /= 1.17,
	setHeight(e, 10, !0, !0),
	(e.top > player.bottom || e.bottom > 360) && unsqueezeBlooper(e)
}
function unsqueezeBlooper(e) {
	e.squeeze = !1,
	removeClass(e, "squeeze"),
	e.counter = 0,
	setHeight(e, 12, !0, !0)
}
function CheepCheep(e, t, n) {
	...
	e.red = t,
	setCheepVelocities(e),
	n ? (x += " jumping",
	e.jumping = !0,
	e.movement = moveCheepJumping) : e.movement = moveCheepInit,
	e.nofall = e.nocollidesolid = e.nocollidechar = !0,
	e.death = killFlip,
	e.collide = collideEnemy,
	setCharacter(e, x),
	TimeHandler.addSpriteCycle(e, ["one", "two"])
...
function setCheepVelocities(e) {
	e.red ? (e.xvel = -unitsized4,
	e.yvel = unitsize / -24) : (e.xvel = unitsize / -6,
	e.yvel = -unitsized32)
}
function moveCheepInit(e) {
	setCheepVelocities(e),
	e.top < player.top && (e.yvel *= -1),
	moveCheep(e),
	e.movement = moveCheep
}
...
function startCheepSpawn() {
	return map.zone_cheeps = TimeHandler.addEventInterval((function() {
			...
			addThing(e, Math.random() * player.left * player.maxspeed / unitsized2, gamescreen.height * unitsize),
			e.xvel = Math.random() * player.maxspeed,
			e.yvel = -2.33 * unitsize,
			flipHoriz(e),
			e.movement = function(e) {
...
function Bubble(e) {
	e.width = e.height = 2,
	e.nofall = e.nocollide = !0,
	e.movement = function(e) {
			e.top < unitsizet16 ? killNormal(e) : shiftVert(e, e.yvel)
	}
	,
	e.yvel = -unitsized4,
	setCharacter(e, "bubble")
}
function Lakitu(e, t) {
	e.width = 8,
	e.height = 12,
	e.nofall = e.noshiftx = e.nocollidesolid = !0,
	e.playerdiff = e.counter = 0,
	e.dir = -1,
	e.norepeat = t,
	e.playerdiff = unitsizet16,
	e.group = "enemy",
	e.collide = collideEnemy,
	e.movement = moveLakituInit,
	e.death = killLakitu,
	setCharacter(e, "lakitu out"),
	map.has_lakitu = e
}
function moveLakituInit(e) {
	if (map.has_lakitu && e.norepeat)
			return killNormal(e);
	TimeHandler.addEventInterval((function(e) {
			if (!e.alive)
					return !0;
...
function moveLakituInit2(e) {
	if (e.right < player.left)
			return moveLakitu(e),
			e.movement = moveLakitu,
			map.lakitu = e,
			!0;
	shiftHoriz(e, -unitsize)
}
function moveLakitu(e) {
	player.xvel > unitsized8 && player.left > gamescreen.width * unitsized2 ? e.left < player.right + unitsizet16 && (slideToXLoc(e, player.right + unitsizet32 + player.xvel, 1.4 * player.maxspeed),
	e.counter = 0) : (e.counter += .007,
	slideToXLoc(e, player.left + player.xvel + 117 * Math.sin(Math.PI * e.counter), .7 * player.maxspeed))
}
function throwSpiny(e) {
	if (!characterIsAlive(e))
			return !1;
	switchClass(e, "out", "hiding"),
	TimeHandler.addEvent((function(e) {
			if (e.dead)
					return !1;
...
function Spiny(e) {
	...
	e.group = "enemy",
	e.speed = .21 * unitsize,
	e.deadly = e.moveleft = !0,
	e.smart = !1,
	e.death = killFlip,
	e.collide = collideEnemy,
	e.movement = moveSimple,
	setCharacter(e, "spiny"),
	TimeHandler.addSpriteCycle(e, ["one", "two"])
}
function SpinyEgg(e) {
	e.height = 8,
	...
	e.group = "enemy",
	e.deadly = !0,
	e.movement = moveSpinyEgg,
	e.spawntype = Spiny,
	e.spawner = e.death = createSpiny,
	e.collide = collideEnemy,
	setCharacter(e, "spinyegg"),
	TimeHandler.addSpriteCycle(e, ["one", "two"])
}
...
function createSpiny(e) {
	...
	addThing(t, e.left, e.top),
	t.moveleft = objectToLeft(player, t),
	killNormal(e)
}
function Beetle(e) {
	e.width = e.height = 8,
	...
	e.smart = !1,
	e.collide = collideEnemy,
	e.movement = moveSmart,
	e.death = killBeetle,
	setCharacter(e, "beetle"),
	TimeHandler.addSpriteCycleSynched(e, ["one", "two"])
}
function killBeetle(e, t) {
	if (e.alive) {
			var n;
			if (n = new Thing(t && 2 != t ? Koopa : BeetleShell,e.smart),
			TimeHandler.addEvent((function(e, t) {
					addThing(e, t.left, t.bottom - e.height * unitsize),
					e.moveleft = t.moveleft
			}
...
function BeetleShell(e) {
	...
	e.death = killFlip,
	e.spawntype = Beetle,
	setCharacter(e, "shell beetle")
}
function Coin(e, t) {
	e.group = "coin",
	e.width = 5,
	e.height = 7,
	e.nofall = e.coin = e.nofire = e.nocollidechar = e.nokillend = e.onlyupsolids = e.skipoverlaps = !0,
	e.tolx = 0,
	e.toly = unitsized2,
	e.collide = hitCoin,
	e.animate = coinEmerge,
	e.death = killNormal,
	setCharacter(e, "coin one"),
	TimeHandler.addSpriteCycleSynched(e, ["one", "two", "three", "two", "one"]),
	t && (e.movement = coinBecomesSolid)
}
function coinBecomesSolid(e) {
	switchContainers(e, characters, solids),
	e.movement = !1
}
function hitCoin(e, t) {
	e.player && (AudioPlayer.play("Coin"),
	score(e, 200, !1),
	gainCoin(),
	killNormal(t))
}
function gainCoin() {
	++data.coins.amount >= 100 && (data.coins.amount = 0,
	gainLife()),
...
function coinEmerge(e, t) {
	...
	score(e, 200, !1),
	gainCoin(),
	e.nocollide = e.alive = e.nofall = e.emerging = !0,
	e.blockparent ? e.movement = coinEmergeMoveParent : e.movement = coinEmergeMove,
	e.yvel = -unitsize,
	TimeHandler.addEvent((function(e) {
			e.yvel *= -1
	}
	...
	TimeHandler.addEvent((function(e) {
			killNormal(e),
			deleteThing(e, scenery, scenery.indexOf(e))
	}
	), 49, e),
	TimeHandler.addEventInterval(coinEmergeMovement, 1, 1 / 0, e, t),
	TimeHandler.clearClassCycle(e, 0),
	addClass(e, "anim"),
	TimeHandler.addSpriteCycle(e, ["anim1", "anim2", "anim3", "anim4", "anim3", "anim2"], 0, 5)
}
...
function Player(e) {
	setPlayerSizeSmall(e),
	e.walkspeed = unitsized2,
	e.canjump = e.nofiredeath = e.nofire = e.player = e.nokillend = 1,
	e.numballs = e.moveleft = e.skidding = e.star = e.dying = e.nofall = e.maxvel = e.paddling = e.jumpers = e.landing = 0,
	e.running = "",
	e.power = data.playerpower,
	e.maxspeed = e.maxspeedsave = 1.35 * unitsize,
	e.scrollspeed = 1.75 * unitsize,
	e.keys = new Keys,
	e.fire = playerFires,
	e.movement = movePlayer,
	e.death = killPlayer,
	setCharacter(e, "player normal small still"),
	e.tolx = unitsizet2,
	e.toly = 0,
	e.gravity = map.gravity,
	map.underwater && (e.swimming = !0,
	TimeHandler.addSpriteCycle(e, ["swim1", "swim2"], "swimming", 5))
}
function placePlayer(e, t) {
	clearOldPlayer(),
	window.player = new Thing(Player),
	window.luigi ? window.player.title = "Luigi" : window.player.title = "Mario";
	var n = addThing(player, e || unitsizet16, t || (map.floor - player.height) * unitsize);
	return data.playerpower >= 2 && (playerGetsBig(player, !0),
	3 == data.playerpower && playerGetsFire(player, !0)),
	n
}
function clearOldPlayer() {
	window.player && (player.alive = !1,
	player.dead = !0)
}
function Keys() {
...
function thingStoreVelocity(e, t) {
	e.xvelOld = e.xvel || 0,
	e.yvelOld = e.yvel || 0,
	e.nofallOld = e.nofall || !1,
	e.nocollideOld = e.nocollide || !1,
	e.movementOld = e.movement || e.movementOld,
	e.nofall = e.nocollide = !0,
	e.xvel = e.yvel = !1,
	t || (e.movement = !1)
}
function thingRetrieveVelocity(e, t) {
	t || (e.xvel = e.xvelOld || 0,
	e.yvel = e.yvelOld || 0),
	e.movement = e.movementOld || e.movement,
	e.nofall = e.nofallOld || !1,
	e.nocollide = e.nocollideOld || !1
}
function removeCrouch() {
	player.crouching = !1,
	player.toly = player.tolyold || 0,
	1 != player.power && (removeClass(player, "crouching"),
	player.height = 16,
	updateBottom(player, 0),
	updateSize(player))
}
function playerShroom(e) {
	e.shrooming || (AudioPlayer.play("Powerup"),
	score(e, 1e3, !0),
	3 != e.power && (e.shrooming = !0,
	(2 == ++e.power ? playerGetsBig : playerGetsFire)(e),
	storePlayerStats()))
}
function playerGetsBig(e, t) {
	if (setPlayerSizeLarge(e),
	e.keys.down = 0,
	...
	t)
			...
			for (var n = [1, 2, 1, 2, 3, 2, 3], x = n.length - 1; x >= 0; --x)
					n[x] = "shrooming" + n[x];
			thingStoreVelocity(player),
			n.push((function(e, t) {
					return e.shrooming = t.length = 0,
					addClass(e, "large"),
					removeClasses(e, "shrooming shrooming3"),
...
function playerGetsSmall(e) {
	...
	TimeHandler.addEvent((function(e) {
			removeClass(e, "large"),
			setPlayerSizeSmall(e),
			setBottom(e, t - unitsize)
	}
	), 21, player),
	TimeHandler.addEvent((function(e) {
			thingRetrieveVelocity(e, !1),
			e.nocollidechar = !0,
			removeClass(e, "paddling"),
			(e.running || e.xvel) && addClass(e, "running"),
			TimeHandler.addEvent(setThingSprite, 1, e)
	}
	), 42, player),
	TimeHandler.addEvent((function(e) {
			e.nocollidechar = !1
...
function movePlayer(e) {
	if (e.keys.up) {
			if (e.keys.jump > 0 && (e.yvel <= 0 || map.underwater) && (map.underwater && playerPaddles(e),
			e.resting ? (e.resting.xvel && (e.xvel += e.resting.xvel),
			e.resting = !1) : (e.jumping || map.underwater || switchClass(e, "running skidding", "jumping"),
			e.jumping = !0),
			!map.underwater)) {
			...
			e.keys.jump = 0;
	e.keys.crouch && !e.crouching && e.resting && (1 != e.power && (e.crouching = !0,
	addClass(e, "crouching"),
	e.height = 11,
	e.tolyold = e.toly,
	e.toly = unitsizet4,
	updateBottom(e, 0),
	updateSize(e)),
	e.resting.actionTop && e.resting.actionTop(e, e.resting, e.resting.transport));
	var n = 0;
	if (0 == e.keys.run || e.crouching)
			e.xvel *= .98,
			n = .035;
	else {
			...
			e.xvel *= .98,
			n = 7e-4,
			signBool(e.keys.run) == e.moveleft ? e.skidding || (addClass(e, "skidding"),
			e.skidding = !0) : e.skidding && (removeClass(e, "skidding"),
			e.skidding = !1)
	}
	e.xvel > n ? e.xvel -= n : e.xvel < -n ? e.xvel += n : 0 != e.xvel && (e.xvel = 0,
	window.nokeys || 0 != e.keys.run || (e.keys.left_down ? e.keys.run = -1 : e.keys.right_down && (e.keys.run = 1))),
	Math.abs(e.xvel) < .14 ? e.running && (e.running = !1,
	1 == player.power && setPlayerSizeSmall(e),
	removeClasses(e, "running skidding one two three"),
	addClass(e, "still"),
	TimeHandler.clearClassCycle(e, "running")) : e.running || (e.running = !0,
	switchClass(e, "still", "running"),
	playerStartRunningCycle(e),
	1 == e.power && setPlayerSizeSmall(e)),
	e.xvel > 0 ? (e.xvel = min(e.xvel, e.maxspeed),
	e.moveleft && (e.resting || map.underwater) && (unflipHoriz(e),
	e.moveleft = !1)) : e.xvel < 0 && (e.xvel = max(e.xvel, -1 * e.maxspeed),
	e.moveleft || !e.resting && !map.underwater || (flipHoriz(e),
	e.moveleft = !0)),
	e.resting && (e.hopping && (removeClass(e, "hopping"),
	e.xvel && addClass(e, "running"),
	e.hopping = !1),
	e.keys.jumplev = e.yvel = e.jumpcount = 0,
	e.jumping && (e.jumping = !1,
	removeClass(e, "jumping"),
	1 == e.power && setPlayerSizeSmall(e),
	addClass(e, abs(e.xvel) < .14 ? "still" : "running")),
	e.paddling && (e.paddling = e.swimming = !1,
	removeClasses(e, "paddling swim1 swim2"),
	TimeHandler.clearClassCycle(e, "paddling"),
	addClass(e, "running"))),
	isNaN(e.xvel)
}
...
function playerPaddles(e) {
	e.paddling || (removeClasses(e, "skidding paddle1 paddle2 paddle3 paddle4 paddle5"),
	addClass(e, "paddling"),
	TimeHandler.clearClassCycle(e, "paddling_cycle"),
	TimeHandler.addSpriteCycle(e, ["paddle1", "paddle2", "paddle3", "paddle3", "paddle2", "paddle1", function() {
			return e.paddling = !1
	}
	], "paddling_cycle", 5)),
	e.paddling = e.swimming = !0,
	e.yvel = -.84 * unitsize
}
...
function movePlayerVine(e) {
	...
	if (e.keys.run == e.attachoff) {
			for (; objectsTouch(e, t); )
					shiftHoriz(e, e.keys.run, !0);
			return unattachPlayer(e)
	}
	if (e.keys.up)
			e.animatednow = !0,
			...
			if (e.animatednow = !0,
			shiftVert(e, unitsized2, !0),
			e.bottom > t.bottom - unitsizet4)
					return unattachPlayer(e)
	} else
			e.animatednow = !1;
	e.animatednow && !e.animated ? addClass(e, "animated") : !e.animatednow && e.animated && removeClass(e, "animated"),
	e.animated = e.animatednow,
	e.bottom < -16 && (locMovePreparations(e),
	!t.locnum && map.random ? goToTransport(["Random", "Sky", "Vine"]) : shiftToLocation(t.locnum))
}
function unattachPlayer(e) {
	e.movement = movePlayer,
	removeClasses(e, "climbing", "animated"),
	TimeHandler.clearClassCycle(e, "climbing"),
	e.yvel = e.skipoverlaps = e.attachoff = e.nofall = e.climbing = e.attached = e.attached.attached = !1,
	e.xvel = e.keys.run
}
function playerHopsOff(e, t, n) {
	removeClasses(e, "climbing running"),
	addClass(e, "jumping"),
	e.piping = e.nocollide = e.nofall = e.climbing = !1,
	e.gravity = gravity / 4,
	e.xvel = 3.5,
	e.yvel = -3.5,
	TimeHandler.addEvent((function(e) {
...
function playerFires() {
	if (!(player.numballs >= 2)) {
			...
			e.yvel = unitsize,
			addThing(e, player.right + unitsized4, player.top + unitsizet8),
			player.moveleft && setRight(e, player.left - unitsized4, !0),
			e.animate(e),
			e.ondelete = fireDeleted,
			TimeHandler.addEvent((function(e) {
					removeClass(e, "firing")
			}
			), 7, player)
...
function playerStar(e) {
	...
	AudioPlayer.playTheme("Star", !0),
	TimeHandler.addEvent(playerRemoveStar, 560, e),
	switchClass(e, "normal", "star"),
	TimeHandler.addSpriteCycle(e, ["star1", "star2", "star3", "star4"], "star", 5))
}
function playerRemoveStar(e) {
	e.star && (--e.star,
	removeClasses(e, "star star1 star2 star3 star4"),
	TimeHandler.clearClassCycle(e, "star"),
	addClass(e, "normal"),
...
function killPlayer(e, t) {
	if (e.alive && !e.flickering && !e.dying) {
			if (2 == t)
					...
					if (!t && e.power > 1)
							return AudioPlayer.play("Power Down"),
							e.power = 1,
							storePlayerStats(),
							playerGetsSmall(e);
					2 != t && (TimeHandler.clearAllCycles(e),
					setSize(e, 7.5, 7, !0),
					updateSize(e),
					setClass(e, "character player dead"),
					nokeys = notime = e.dying = !0,
					thingStoreVelocity(e),
					containerForefront(e, characters),
					TimeHandler.addEvent((function(e) {
							thingRetrieveVelocity(e, !0),
							e.nocollide = !0,
							e.movement = e.resting = !1,
							e.gravity = gravity / 2.1,
							e.yvel = -1.4 * unitsize
					}
					), 7, e))
			...
			AudioPlayer.pause(),
			window.editing || AudioPlayer.play("Player Dies"),
			e.nocollide = e.nomove = nokeys = 1,
			--data.lives.amount,
			map.random || (data.score.amount = data.scoreold),
			window.editing ? setTimeout((function() {
					editorSubmitGameFuncPlay(),
					editor.playing = editor.playediting = !0
			}
			), 35 * timer) : !map.random || data.lives.amount <= 0 ? window.reset = setTimeout(data.lives.amount ? setMap : gameOver, 280 * timer) : (nokeys = notime = !1,
			updateDataElement(data.score),
			updateDataElement(data.lives),
			TimeHandler.addEvent((function() {
					playerDropsIn(),
...
function playerDropsIn() {
	clearOldPlayer(),
	placePlayer(unitsizet16, -1 * unitsizet8 + map.underwater * unitsize * 24),
	flicker(player),
	map.underwater ? player.gravity = gravity / 2.8 : (player.nocollide = !0,
	TimeHandler.addEvent((function() {
			player.nocollide = !1,
...
function Brick(e, t) {
	if (e.width = e.height = 8,
	e.used = !1,
	e.bottomBump = brickBump,
	t)
			if (t instanceof Array)
					for (e.contents = t; e.contents.length < 3; )
							e.contents.push(!1);
			else
					e.contents = [t, !1, !1];
	else
			e.contents = !1;
	e.death = killNormal,
	setSolid(e, "brick unused"),
	e.tolx = 1
}
function brickBump(e, t) {
	if (!e.up && "player" == t.type && (AudioPlayer.play("Bump"),
	!e.used)) {
			if (e.up = t,
			t.power > 1 && !e.contents)
					return TimeHandler.addEvent(brickBreak, 2, e, t);
			blockBumpMovement(e),
			e.contents && (player.power > 1 && e.contents[0] == Mushroom && !e.contents[1] && (e.contents[0] = FireFlower),
			TimeHandler.addEvent((function(e) {
					var t = e.contents
						, n = new Thing(t[0],t[1],t[2]);
					addThing(n, e.left, e.top),
					setMidXObj(n, e, !0),
					n.blockparent = e,
					n.animate(n, e),
					e.contents[0] == Coin ? (e.lastcoin && makeUsedBlock(e),
					TimeHandler.addEvent((function(e) {
							e.lastcoin = !0
					}
...
function makeUsedBlock(e) {
	e.used = !0,
	switchClass(e, "unused", "used")
}
function brickBreak(e, t) {
	AudioPlayer.play("Break Block"),
	score(e, 50),
	e.up = t,
	TimeHandler.addEvent(placeShards, 1, e),
	killNormal(e)
}
function placeShards(e) {
	for (var t, n = 0; n < 4; ++n)
			addThing(t = new Thing(BrickShard), e.left + (n < 2) * e.width * unitsize - unitsizet2, e.top + n % 2 * e.height * unitsize - unitsizet2),
			t.xvel = unitsized2 - unitsize * (n > 1),
			t.yvel = -1.4 * unitsize + n % 2,
			TimeHandler.addEvent(killNormal, 350, t)
}
function BrickShard(e) {
	e.width = e.height = 4,
	e.nocollide = !0,
	e.death = killNormal,
	setCharacter(e, "brickshard"),
	TimeHandler.addSpriteCycle(e, [unflipHoriz, flipHoriz])
}
function attachEmerge(e, t) {
	e.animate = setInterval((function() {
			setBottom(e, t.top, !0),
			t.up || (clearInterval(e.animate),
			e.animate = !1)
	}
	), timer)
}
function Block(e, t, n) {
	...
	t)
			...
			e.contents = [Coin];
	e.death = killNormal,
	setSolid(e, "Block unused"),
	e.hidden = !!n && (e.hidden = e.skipoverlaps = !0),
	e.tolx = 1,
	TimeHandler.addSpriteCycleSynched(e, ["one", "two", "three", "two", "one"])
}
function blockBump(e, t) {
	"player" == t.type && (e.used ? AudioPlayer.play("Bump") : (e.used = 1,
	e.hidden = e.hidden = e.skipoverlaps = !1,
	e.up = t,
	blockBumpMovement(e),
	removeClass(e, "hidden"),
	switchClass(e, "unused", "used"),
	player.power > 1 && e.contents[0] == Mushroom && !e.contents[1] && (e.contents[0] = FireFlower),
	TimeHandler.addEvent(blockContentsEmerge, 7, e)))
}
...
function Pipe(e, t, n) {
	e.width = e.spritewidth = 16,
	e.height = 8 * (t || 1),
	!1 !== n && (e.actionTop = intoPipeVert,
	e.transport = n),
	setSolid(e, "pipe")
}
function PipeSide(e, t, n) {
	e.width = e.spritewidth = n ? 8 : 19.5,
	e.height = e.spriteheight = 16,
	t && (e.actionLeft = intoPipeHoriz,
	e.transport = t),
	setSolid(e, "pipe side " + (n ? "small" : ""))
}
function PipeVertical(e, t) {
	e.spritewidth = e.width = 16,
	e.spriteheight = e.repeat = 1,
	e.height = t,
	setSolid(e, "pipe vertical")
}
function Vine(e, t) {
	e.width = e.spriteheight = 7,
	e.height = 0,
	e.locnum = t,
	e.nocollide = e.nofall = e.repeat = !0,
	e.animate = vineEmerge,
	e.movement = vineMovement,
	setCharacter(e, "vine")
}
function vineEmerge(e, t) {
	...
	setHeight(e, 0),
	e.movement = vineMovement,
	TimeHandler.addEvent(vineEnable, 14, e),
	TimeHandler.addEventInterval(vineStay, 1, 14, e, t)
}
...
function vineMovement(e) {
	increaseHeightTop(e, unitsized4),
	e.attached && shiftVert(e.attached, -unitsized4, !0)
}
function touchVine(e, t) {
	!e.player || e.attached || e.climbing || e.bottom > t.bottom + unitsizet2 || (t.attached = e,
	e.attached = t,
	e.nofall = e.skipoverlaps = !0,
	e.xvel = e.yvel = e.resting = e.jumping = e.jumpcount = e.running = 0,
	e.attachleft = !objectToLeft(e, t),
	e.attachoff = 2 * e.attachleft - 1,
	e.movementsave = e.movement,
	e.movement = movePlayerVine,
	e.keys = new Keys,
	TimeHandler.clearClassCycle(e, "running"),
	removeClass(e, "running skidding"),
	unflipHoriz(e),
	e.attachleft && flipHoriz(e),
	addClass(e, "climbing"),
	e.climbing = TimeHandler.addSpriteCycle(e, ["one", "two"], "climbing"),
	lookTowardThing(e, t),
	e.attachleft ? setLeft(e, t.right - unitsizet4) : setRight(e, t.left + unitsizet4))
}
function Springboard(e) {
	e.width = 8,
	e.height = e.heightnorm = 14.5,
	e.tension = e.tensionsave = 0,
	e.dir = 1,
	e.collide = collideSpring,
	setSolid(e, "springboard")
}
...
function springPlayerInit(e, t) {
	e.tension = e.tensionsave = max(.77 * t.yvel, unitsize),
	t.movement = movePlayerSpringDown,
	t.spring = e,
	t.xvel /= 2.8
}
function movePlayerSpringDown(e) {
	...
	void (e.spring.movement = moveSpringUp)) : ((e.left < e.spring.left + unitsizet2 || e.right > e.spring.right - unitsizet2) && (e.xvel /= 1.4),
	reduceSpringHeight(e.spring, e.spring.tension),
	setBottom(e, e.spring.top, !0),
	e.spring.tension /= 2,
	void updateSize(e.spring)) : (e.movement = movePlayer,
	e.spring.movement = moveSpringUp,
	void (e.spring = !1))
}
function movePlayerSpringUp(e) {
	if (!e.spring || !objectsTouch(e, e.spring))
			return e.spring = !1,
			void (e.movement = movePlayer)
}
function moveSpringUp(e) {
	reduceSpringHeight(e, -e.tension),
	e.tension *= 2,
	e == player.spring && setBottom(player, e.top, !0),
	e.height > e.heightnorm && (e == player.spring && (player.yvel = max(-unitsizet2, -.98 * e.tensionsave),
	player.resting = player.spring = !1),
	reduceSpringHeight(e, (e.height - e.heightnorm) * unitsize),
	e.tension = e.tensionsave = e.movement = !1)
}
...
function RestingStone(e) {
	e.width = e.height = 8,
	e.used = !1,
	e.movement = RestingStoneUnused,
	setSolid(e, "Stone hidden"),
	e.title = "Stone"
}
...
function CastleBlock(e, t, n) {
	e.width = e.height = 8;
	var x, r, i = !1;
	t instanceof Array ? (x = t[0],
	r = t[1],
	i = n) : (x = t,
	r = n),
	setSolid(e, i ? "castleblockinvis" : "castleblock"),
	x && (e.balls = new Array(x),
	e.dt = .07 * (r ? 1 : -1),
	e.timeout = round(7 / (abs(r) || 1)),
	e.movement = castleBlockSpawn,
	e.timer = e.counter = 0,
	e.angle = .25,
	e.spawn_as_char = !0)
}
...
function castleBlockEvent(e) {
	...
	e.counter = 0,
	e.angle += e.dt;
	for (var t = 1; t < e.balls.length; ++t)
			setMidX(e.balls[t], e.midx + t * unitsizet4 * Math.cos(e.angle * Math.PI), !0),
			setMidY(e.balls[t], e.midy + t * unitsizet4 * Math.sin(e.angle * Math.PI), !0)
}
function CastleFireBall(e, t) {
	e.width = e.height = 4,
	e.deadly = e.nofire = e.nocollidechar = e.nocollidesolid = e.nofall = e.nostar = e.outerok = e.skipoverlaps = !0,
	e.movement = !1,
	e.collide = collideEnemy,
	setCharacter(e, "fireball castle"),
	TimeHandler.addSpriteCycle(e, ["one", "two", "three", "four"], 4)
}
function CastleBridge(e, t) {
	...
	e.spritewidth = 4,
	e.repeat = !0,
	setSolid(e, "CastleBridge")
}
function CastleChain(e) {
	e.height = 8,
	e.width = e.spritewidth = 7.5,
	e.nocollide = !0,
	setSolid(e, "castlechain")
}
function CastleAxe(e) {
	e.width = e.height = 8,
	e.spritewidth = e.spriteheight = 8,
	e.nocollide = !0,
	setSolid(e, "castleaxe"),
	TimeHandler.addSpriteCycle(e, ["one", "two", "three", "two"])
}
function CastleAxeFalls(e, t) {
	var n = t.axe;
	!e.player || e.right < n.left + unitsize || e.bottom > n.bottom - unitsize || (killNormal(n),
	killNormal(t),
	notime = nokeys = !0,
	thingStoreVelocity(e),
	killOtherCharacters(),
	TimeHandler.addEvent(killNormal, 7, n.chain),
	TimeHandler.addEvent(CastleAxeKillsBridge, 14, n.bridge, n),
	AudioPlayer.pauseTheme(),
	AudioPlayer.playTheme("World Clear", !1, !1))
}
function CastleAxeKillsBridge(e, t) {
	e.width -= 2,
	e.right -= unitsizet2,
	e.width > 0 ? TimeHandler.addEvent(CastleAxeKillsBridge, 1, e, t) : (e.width = 0,
	TimeHandler.addEvent(CastleAxeKillsBowser, 1, t.bowser)),
	setWidth(e, e.width)
}
...
function collideCastleNPC(e, t) {
	...
	TimeHandler.addEvent((function(e) {
			...
			for (t = 0; t < e.length; ++t)
					TimeHandler.addEvent(proliferate, 70 * t, e[t].element, {
							style: {
									visibility: "visible"
							}
					});
...
function Platform(e, t, n) {
	...
	e.spritewidth = 4,
	e.moving = 0,
	e.repeat = e.killonend = !0,
	"function" == typeof n && (n = [n]),
	n instanceof Array && (e.movement = n[0],
	e.begin = n[1] * unitsize,
	e.end = n[2] * unitsize,
	e.maxvel = (n[3] || 1.5) * unitsized4,
	e.movement == moveFloating || e.movement == movePlatformSpawn ? e.yvel = e.maxvel : e.xvel = e.maxvel,
	e.changing = 0),
	e.movement == collideTransport && (e.movement = !1,
	e.collide = collideTransport),
	setSolid(e, "platform")
}
function PlatformGenerator(e, t, n) {
	e.width = 4 * t,
	e.interval = 35,
	e.height = 6 * e.interval,
	e.dir = n,
	e.nocollide = e.hidden = !0,
	e.movement = PlatformGeneratorInit,
	setSolid(e, "platformgenerator")
}
function PlatformGeneratorInit(e) {
	for (var t = 0, n = e.interval, x = e.height; t < x; t += n)
			e.platlast = new Thing(Platform,e.width / 4,[movePlatformSpawn, 0, 0, 1.5]),
			e.platlast.yvel *= e.dir,
			1 == e.dir ? addThing(e.platlast, e.left, e.top + t * unitsize) : addThing(e.platlast, e.left, e.bottom - t * unitsize),
			e.platlast.parent = e,
			t += e.interval;
	e.movement = !1
}
function movePlatformSpawn(e) {
	e.bottom < e.parent.top ? (setBottom(e, e.parent.bottom),
	detachPlayer(e)) : e.top > e.parent.bottom ? (setTop(e, e.parent.top),
	detachPlayer(e)) : movePlatformNorm(e)
}
function movePlatformNorm(e) {
	shiftHoriz(e, e.xvel),
	shiftVert(e, e.yvel),
	e == player.resting && e.alive && (setBottom(player, e.top),
	shiftHoriz(player, e.xvel),
	player.right > innerWidth && setRight(player, innerWidth))
}
...
function Scale(e, t, n) {
	e.height = 5,
	e.width = 4 * t,
	e.spritewidth = e.spriteheight = 5,
	e.repeat = e.nocollide = !0,
	setSolid(e, "scale")
}
function Flag(e) {
	e.width = e.height = 8,
...
function FlagPole(e) {
	e.width = 1,
	e.height = 72,
	e.nocollide = e.repeat = !0,
	setSolid(e, "flagpole")
}
function FlagTop(e) {
	e.spritewidth = e.spriteheight = e.width = e.height = 4,
	e.nocollide = !0,
	setSolid(e, "flagtop")
}
function FlagDetector(e) {
	e.width = 2,
	e.height = 100,
	e.collide = FlagCollision,
	setSolid(e, "flagdetector"),
	e.hidden = !0
}
function CastleDoorDetector(e) {
	e.width = e.height = 4,
	e.collide = endLevelPoints,
	setSolid(e, "castledoor"),
	e.hidden = !0
}
function FlagCollision(e, t) {
	...
	player.nocollidechar = !0,
	setRight(e, t.pole.left, !0),
	removeClasses(e, "running jumping skidding"),
	addClass(e, "climbing animated"),
	updateSize(e),
	TimeHandler.addSpriteCycle(e, ["one", "two"], "climbing"),
	playerRemoveStar(player);
	var n = !1
		, x = !1
		, r = (t.stone.top - e.bottom) / unitsize
		, i = setInterval((function() {
			n || (e.bottom >= t.stone.top ? (scorePlayerFlag(r, t.stone),
			n = !0,
			setBottom(e, t.stone.top, !0),
			removeClass(player, "animated"),
			TimeHandler.clearClassCycle(player, "climbing")) : shiftVert(e, unitsize, !0)),
			x || (t.flag.bottom >= t.stone.top ? (x = !0,
			setBottom(t.flag, t.stone.top, !0)) : shiftVert(t.flag, unitsize, !0)),
			n && x && (setBottom(e, t.stone.top, !0),
			clearInterval(i),
			setTimeout((function() {
					FlagOff(e, t.pole)
			}
...
function scorePlayerFlag(e, t) {
	var n;
	n = e < 28 ? e < 8 ? 100 : 400 : e < 40 ? 800 : e < 62 ? 2e3 : 5e3,
	score(player, n, !0)
}
...
function endLevelPoints(e, t) {
	if (e && e.player) {
			...
			var x = setInterval((function() {
					...
					updateDataElement(data.time),
					AudioPlayer.play("Coin"),
					data.time.amount <= 0 && (clearInterval(x),
					setTimeout((function() {
...
function endLevelFireworks(e, t, n) {
	var x, r, i = 0;
	if (t) {
			for (var a = n.left + 32 * unitsized2; i < t; )
					explodeFirework(++i, a);
			x = timer * (i + 2) * 42
	} else
			x = 0;
...
function explodeFirework(e, t) {
	setTimeout((function() {
			...
			addThing(n, t + n.locs[0] - 6 * unitsize, unitsizet16 + n.locs[1]),
			n.animate()
	}
	), timer * e * 42)
}
function Firework(e, t) {
	if (e.width = e.height = 8,
	e.nocollide = e.nofire = e.nofall = !0,
	t)
			switch (t) {
			case 1:
	...
	e.animate = function() {
			var t = e.className + " n";
			e.locs && AudioPlayer.play("Firework"),
			TimeHandler.addEvent((function(e) {
					setClass(e, t + 1)
			}
			), 0, e),
...
function BridgeBase(e, t) {
	...
	e.width = 8 * t,
	e.repeat = !0,
	setSolid(e, "bridge-base")
}
function WarpWorld(e) {
	e.width = 106,
	e.height = 88,
	e.movement = setWarpWorldInit,
	e.collide = enableWarpWorldText,
	e.pirhanas = [],
	e.pipes = [],
	e.texts = [],
	e.hidden = !0,
	setSolid(e, "warpworld")
}
function setWarpWorldInit(e) {
	shiftHoriz(e, e.width * unitsized2),
	e.width /= 2,
	updateSize(e),
	e.movement = !1
}
function enableWarpWorldText(e, t) {
	var n, x = t.pirhanas, r = t.texts;
	for (n in x)
			x[n].death();
	for (n in r)
			r[n].element.style.visibility = "";
	killNormal(t)
}
function resetScenery() {
	window.Scenery = {
			...
			patterns: {
					backreg: [["HillLarge", 0, 0], ["Cloud1", 68, 68], ["Bush3", 92, 0], ["HillSmall", 128, 0], ["Cloud1", 156, 76], ["Bush1", 188, 0], ["Cloud3", 220, 68], ["Cloud2", 292, 76], ["Bush2", 332, 0], ["Blank", 384]],
					backcloud: [["Cloud2", 28, 64], ["Cloud1", 76, 32], ["Cloud2", 148, 72], ["Cloud1", 228, 0], ["Cloud1", 284, 32], ["Cloud1", 308, 40], ["Cloud1", 372, 0], ["Blank", 384]],
					backcloudmin: [["Cloud1", 68, 68], ["Cloud1", 156, 76], ["Cloud3", 220, 68], ["Cloud2", 292, 76], ["Blank", 384]],
					backfence: [["PlantSmall", 88, 0], ["PlantLarge", 104, 0], ["Fence", 112, 0, 4], ["Cloud1", 148, 68], ["PlantLarge", 168, 0], ["PlantSmall", 184, 0], ["PlantSmall", 192, 0], ["Cloud1", 220, 76], ["Cloud2", 244, 68], ["Fence", 304, 0, 2], ["PlantSmall", 320, 0], ["Fence", 328, 0], ["PlantLarge", 344, 0], ["Cloud1", 364, 76], ["Cloud2", 388, 68], ["Blank", 384]],
					backfencemin: [["PlantLarge", 104, 0], ["Fence", 112, 0, 4], ["Cloud1", 148, 68], ["PlantLarge", 168, 0], ["PlantSmall", 184, 0], ["PlantSmall", 192, 0], ["Cloud1", 220, 76], ["Cloud2", 244, 68], ["Fence", 304, 0, 2], ["PlantSmall", 320, 0], ["Fence", 328, 0], ["Cloud1", 364, 76], ["Cloud2", 388, 68], ["Blank", 384]],
					backfencemin2: [["Cloud2", 4, 68], ["PlantSmall", 88, 0], ["PlantLarge", 104, 0], ["Fence", 112, 0, 1], ["Fence", 128, 0, 2], ["Cloud1", 148, 68], ["PlantSmall", 184, 0], ["PlantSmall", 192, 0], ["Cloud1", 220, 76], ["Cloud2", 244, 68], ["Fence", 304, 0, 2], ["PlantSmall", 320, 0], ["Fence", 328, 0], ["PlantLarge", 344, 0], ["Cloud1", 364, 76], ["Cloud2", 388, 68], ["Blank", 384]],
					backfencemin3: [["Cloud2", 4, 68], ["PlantSmall", 88, 0], ["PlantLarge", 104, 0], ["Fence", 112, 0, 4], ["Cloud1", 148, 68], ["PlantSmall", 184, 0], ["PlantSmall", 192, 0], ["Cloud1", 220, 76], ["Cloud2", 244, 68], ["Cloud1", 364, 76], ["Cloud2", 388, 68], ["Blank", 384]]
			}
	},
...
function processSceneryPatterns(e) {
	var t, n;
	for (n in e)
			(t = e[n]).length && (t.width = t[t.length - 1][1],
			t.pop())
}
function SceneryBlocker(e, t, n) {
	...
	e.height = n || 8,
	e.nocollide = e.hidden = !0,
	setSolid(e, "sceneryblocker")
}
function Sprite(e, t, n) {
	n || (n = [1, 1]);
	var x = e.template = Scenery.sprites[t];
	x ? (e.width = (e.spritewidth = x[0]) * (n[0] || 1),
	e.height = (e.spriteheight = x[1]) * (n[1] || 1),
	e.unitwidth = e.spritewidth * unitsize,
	e.unitheight = e.spriteheight * unitsize,
	e.nocollide = e.maxquads = 1,
	e.repeat = !0,
	setScenery(e, "scenery " + t),
	e.title = t,
	x.spriteCycleTimer && TimeHandler.addSpriteCycle(e, spriteCycleTimer, spriteCycleTimer || void 0)) : log("No sprite template found for", t)
}
function LocationShifter(e, t, n) {
	e.loc = t,
	e.width = n[0],
	e.height = n[1],
	e.collide = collideLocationShifter,
	e.hidden = !0,
	setSolid(e, "blue")
}
function collideLocationShifter(e, t) {
	e.player && (t.nocollide = player.piping = !0,
	TimeHandler.addEvent((function(e) {
			shiftToLocation(t.loc),
			map.random && entryRandom(e)
	}
	), 1, e))
}
function ScrollBlocker(e, t) {
	...
	e.big = t,
	e.movement = function() {
			e.left - player.xvel <= gamescreen.right - gamescreen.left && (map.canscroll = e.movement = !1,
			map.noscroll = e.big)
	}
	,
...
function GenerationStarter(e, t, n) {
	...
	e.movement = function(e) {
			e.movement = !1,
			addClass(e, "used"),
			e.func((gamescreen.left + e.right) / unitsize, e.arg)
	}
	,
	setSolid(e, "generationstarter"),
	e.hidden = !0
}
function castleDecider(e, t, n) {
	...
	e.nocollide = !0,
	e.xloc = t,
	e.section = map.area.sections[n],
	e.next = map.area.sections[n + 1],
	e.movement = function(e) {
			if (!(e.left > gamescreen.right - gamescreen.left) && e.section.activated) {
					var t = e.section;
					t.numpass = t.colliders.length = 0,
					t.passed ? (++map.area.sections.current,
					e.next(e.xloc)) : t(e.xloc),
					t.activated = t.passed = !1,
					spawnMap(),
					killNormal(e)
			}
	}
	,
	setSolid(e, "decider blue " + n),
	e.hidden = !0
}
function FuncCollider(e, t, n) {
	n ? (e.width = n[0],
	e.height = n[1]) : (e.width = 8,
	e.height = ceilmax + 40),
	e.collide = t,
	e.hidden = !0,
	setSolid(e, "funccollider blue " + t.name)
}
...
function Collider(e, t, n) {
	e.width = t[0],
	e.height = t[1],
	n instanceof Array ? (e.func = n[0] || function() {}
	,
	e.movement = n[1] || function() {}
	) : (e.func = n || function() {}
	,
	e.movement = !1),
	e.collide = function(e, t) {
	...
	,
	setSolid(e, "collider blue " + e.func.name),
	e.hidden = !0
}
function TonedJS(e) {
	var t = {
			...
			proliferate: function(e, t, n) {
					var x, r;
					for (r in t)
							n && e.hasOwnProperty(r) || ("object" == typeof (x = t[r]) && null != x ? (e.hasOwnProperty(r) || (e[r] = x instanceof Array ? [] : {}),
							proliferate(e[r], x, n)) : e[r] = x);
					return e
			},
			...
			createElement: function(e) {
					for (var t = document.createElement(e || "div"), n = arguments.length; --n > 0; )
							proliferate(t, arguments[n]);
					return t
			},
			...
			elementShiftLeft: function(e, t) {
					e.left || (e.left = Number(e.style.marginLeft.replace("px", ""))),
					e.style.marginLeft = round(e.left += t) + "px"
			},
			elementShiftTop: function(e, t) {
					e.top || (e.top = Number(e.style.marginLeft.replace("px", ""))),
					e.style.marginTop = round(e.top += t) + "px"
			},
			...
			capitalizeFirst: function(e, t) {
					return t = t || 1,
					e.substr(0, t).toUpperCase() + e.substr(t).toLowerCase()
			},
			ArrayD: function(e) {
					if (1 == arguments.length)
							return new Array(e);
					var t, n = arrayMake(arguments), x = new Array(e);
					for (n.shift(),
					t = e - 1; t >= 0; --t)
							x[t] = ArrayD.apply(this, n);
					return x
			},
			...
			arrayRange: function(e, t) {
					for (var n = 1 + t - e, x = new Array(n), r = e, i = 0; i < n; )
							x[i++] = r++;
					return x
			},
			...
			makeDigit: function(e, t, n) {
					return e = String(e),
					stringOf(n || 0, max(0, t - e.length)) + e
			},
...
function resetTriggers() {
	...
	gamepad.bind(Gamepad.Event.BUTTON_UP, ControlsPipe("keyup", !1)),
	gamepad.bind(Gamepad.Event.AXIS_CHANGED, (function(e) {
			...
			if (!(n < .1))
					...
					case "LEFT_STICK_Y":
					case "RIGHT_STICK_Y":
							n > .5 ? keydown(t > 0 ? "DPAD_DOWN" : "DPAD_UP") : (keyup("DPAD_UP"),
							keyup("DPAD_DOWN"));
							break;
					case "LEFT_STICK_X":
					case "RIGHT_STICK_X":
							n > .5 ? keydown(t < 0 ? "DPAD_LEFT" : "DPAD_RIGHT") : (keyup("DPAD_UP"),
							keyup("DPAD_DOWN"))
					}
	}
	)),
...
function Controls(e, t) {
	...
	var n, x, r, i, a = this.keydown = {
			...
			up: function(e) {
					e.up = !0,
					player.canjump && (player.resting || map.underwater) && (e.jump = 1,
					player.canjump = e.jumplev = 0,
					AudioPlayer.play(player.power > 1 ? "Jump Super" : "Jump Small"),
					map.underwater && setTimeout((function() {
			...
			sprint: function(e) {
					3 != player.power || 0 != e.sprint || e.crouch || player.fire(),
					e.sprint = 1
			},
			pause: function(e) {
...
function ControlsPipe(e, t) {
	var n = controls[e];
	return function(x) {
			t && (player && player.dead || window.paused) || window.nokeys || (("number" != typeof x || x.which || x.control) && (x = x.which || x.control),
			n[x] ? n[x](player.keys) : mlog(e, "Could not", e, x),
			window.gamehistory[gamecount] = [keydown, x])
	}
}
function keydown(e) {
	if (!(player && player.dead || window.paused || window.nokeys)) {
			var t = controls.keydown;
			("object" == typeof e || e.which) && (e = e.which),
			t[e] && t[e](player.keys),
			window.gamehistory[gamecount] = [keydown, e]
	}
}
function keyup(e) {
	if (!window.nokeys) {
			...
			("object" == typeof e || e.which) && (e = e.which),
			t[e] && t[e](player.keys),
			window.gamehistory[gamecount] = [keyup, e]
	}
}
...
function mousedown(e) {
	3 == e.which && (paused ? unpause() : window.editor && (editing || editor.playing) || pause(!0),
	e.preventDefault && e.preventDefault())
}
function scriptKeys(e) {
	...
	for (t in e)
			n = e[t],
			TimeHandler.addEvent(n[0], t, n[1]),
			TimeHandler.addEvent((function() {
					alert(n[0].name + ", " + n[1])
			}
			), t)
...
function lulz(e, t) {
	...
	TimeHandler.addEventInterval((function() {
			if (!(characters.length > 210)) {
					...
					t.yvel = random() * -unitsizet4,
					t.xvel = t.speed = random() * unitsizet2 * randSign(),
					addThing(t, (32 * random() + 128) * unitsize, 88 * random() * unitsize)
			}
	}
	), t, 1 / 0)
}
...
function mapKeyToControl(e, t) {
	if (-1 == window.controls.pipes[e].indexOf(t)) {
			...
			window.controls = new Controls(n),
			proliferate(body, {
					onkeydown: ControlsPipe("keydown", !0),
					onkeyup: ControlsPipe("keyup", !1),
					oncontextmenu: contextmenu,
...
function setMessageTriggers() {
	var e = {
			...
			toggleOption: function(e) {
					...
					window[t] ? window[t]() : log("Could not toggle", t)
			},
			setKey: function(e) {
...
function triggerSetMap(e) {
	clearPlayerStats(),
	setMap.apply(this, e.map || []),
	setLives(3)
}
function upkeep() {
	if (!window.paused) {
			window.nextupk = setTimeout(upkeep, timer);
			for (var e = window.speed; e > 0; --e)
					adjustFPS(),
...
function adjustFPS() {
	...
	window.fps = roundDigit(.7 * fps + .3 * t, .01),
	window.realtime = fps_target / fps,
	window.time_prev = time_now
}
function pause(e) {
	paused && !window.nextupk || (cancelAnimationFrame(nextupk),
	AudioPlayer.pause(),
	paused = !0,
	e && AudioPlayer.play("Pause"))
}
function unpause() {
	paused && (window.nextupk = requestAnimationFrame(upkeep),
	paused = !1,
	AudioPlayer.resume())
}
function maintainSolids(e) {
	for (var t, n = 0; n < solids.length; ++n)
			(t = solids[n]).alive && t.movement && t.movement(t),
			(!t.alive || t.right < QuadsKeeper.getDelX()) && deleteThing(t, solids, n)
}
function maintainCharacters(e) {
	var t, n, x = gamescreen.right + QuadsKeeper.getOutDifference();
	for (n = 0; n < characters.length; ++n)
			(t = characters[n]).resting ? t.yvel = 0 : (t.nofall || (t.yvel += t.gravity || map.gravity),
			t.yvel = min(t.yvel, map.maxyvel)),
			updatePosition(t),
			QuadsKeeper.determineThingQuadrants(t),
			t.under = t.undermid = !1,
			determineThingCollisions(t),
			t.resting && (characterOnResting(t, t.resting) ? (t.yvel = !1,
			setBottom(t, t.resting.top)) : t.resting = !1),
			t.alive ? "player" == t.type || map.shifting || !(0 == t.numquads || t.left > x) || t.outerok ? !t.nomove && t.movement && t.movement(t) : deleteThing(t, characters, n) : map.shifting || deleteThing(t, characters, n)
}
function maintainPlayer(e) {
	var t = !1;
	if (player.alive) {
			if (player.yvel > 0 && (map.underwater || (player.keys.jump = 0),
			player.jumping || (map.underwater ? player.paddling || (switchClass(player, "paddling", "paddling"),
			player.padding = !0) : (addClass(player, "jumping"),
			player.jumping = !0)),
			!player.piping && !player.dying && player.top > gamescreen.deathheight)) {
					if (map.exitloc)
							return map.random ? (goToTransport(["Random", "Overworld", "Down"]),
							void playerDropsIn()) : shiftToLocation(map.exitloc);
					clearPlayerStats(),
					killPlayer(player, 2)
			}
			player.xvel > 0 ? player.right > gamescreen.middlex && (t = !0,
			window.firstMiddle = !0,
			player.right > gamescreen.right - gamescreen.left && (player.xvel = min(0, player.xvel))) : player.left < 0 && (player.xvel = max(0, player.xvel)),
			player.under && (player.jumpcount = 0),
			window.scrolloffset = map.canscroll * (player.right - gamescreen.middlex),
			is_mobile ? (map.canscroll = !0,
			window.firstMiddle && !map.shifting && window.sumLeftScroll >= -220 || player.xvel > 0 && t ? (scrollWindow(lastscroll = round(min(player.scrollspeed, scrolloffset))),
			(window.sumLeftScroll <= 0 || window.sumLeftScroll > 0 && scrolloffset < 0) && (window.sumLeftScroll = window.sumLeftScroll + lastscroll)) : lastscroll = 0) : scrolloffset > 0 && !map.shifting ? scrollWindow(lastscroll = round(min(player.scrollspeed, scrolloffset))) : lastscroll = 0
	}
}
function maintainTexts() {
	var e, t, n;
	for (n = texts.length - 1; n >= 0; --n)
			e = (t = texts[n]).element || t,
			t.xvel && elementShiftLeft(e, t.xvel),
			t.yvel && elementShiftTop(e, t.yvel)
}
...
function getCanvas(e, t, n) {
	...
	return n && (n = n || unitsize,
	proliferate(x.style, {
			width: e * n + "px",
			height: t * n + "px"
	})),
	x.getContext("2d").webkitImageSmoothingEnabled = !1,
	x
}
function step(e) {
	unpause(),
...
function toggleFastFWD(e) {
	window.fastforwarding ? (fastforward(0),
	window.fastforwarding = !1) : (fastforward(2),
	window.fastforwarding = !0)
}
...
function updatePosition(e) {
	e.nomove || shiftHoriz(e, e.xvel),
	e.nofall || shiftVert(e, e.yvel)
}
function updateSize(e) {
	var t;
	e.unitwidth = e.width * unitsize,
	e.unitheight = e.height * unitsize,
	e.spritewidthpixels = e.spritewidth * unitsize,
	e.spriteheightpixels = e.spriteheight * unitsize,
	(t = e.canvas) && (t.width = e.spritewidthpixels,
	t.height = e.spriteheightpixels,
	refillThingCanvas(e))
}
...
function shiftBoth(e, t, n) {
	e.noshiftx || shiftHoriz(e, t),
	e.noshifty || shiftVert(e, n)
}
...
function setLeft(e, t) {
	e.left = t,
	e.right = e.left + e.width * unitsize
}
function setRight(e, t) {
	e.right = t,
	e.left = e.right - e.width * unitsize
}
function setTop(e, t) {
	e.top = t,
	e.bottom = e.top + e.height * unitsize
}
function setBottom(e, t) {
	e.bottom = t,
	e.top = e.bottom - e.height * unitsize
}
function setWidth(e, t, n, x) {
	e.width = t,
	e.unitwidth = t * unitsize,
	n && (e.spritewidth = t,
	e.spritewidthpixels = t * unitsize),
	x && (updateSize(e),
	setThingSprite(e))
}
function setHeight(e, t, n, x) {
	e.height = t,
	e.unitheight = t * unitsize,
	n && (e.spriteheight = t,
	e.spriteheightpixels = t * unitsize),
	x && (updateSize(e),
	setThingSprite(e))
}
...
function determineThingCollisions(e) {
	if (!e.nocollide) {
			var t, n, x, r, i, a, o;
			for (e.resting && 0 != e.resting.yvel || (e.resting = !1),
			e.skipoverlaps || checkOverlap(e),
			r = 0,
			a = e.numquads; r < a; ++r)
					for (n = (t = e.quadrants[r]).things,
					i = 0,
					o = t.numthings; i < o && e != (x = n[i]); ++i)
							!x.alive || x.scenery || x.nocollide || !objectsTouch(e, x) || !e.player && x.hidden && x.visual_scenery && x.visual_scenery.hidden && !solidOnCharacter(x, e) || (x.character ? objectsCollided(e, x) : e.nocollidesolid || (objectsCollided(e, x),
							e.skipoverlaps || x.skipoverlaps || !characterOverlapsSolid(e, x) || e.overlaps.push(x)));
...
function checkOverlap(e) {
	if (e.overlapdir)
			e.overlapdir < 0 && e.right <= e.ocheck.left + unitsizet2 || e.left >= e.ocheck.right - unitsizet2 ? (e.overlapdir = 0,
			e.overlaps = []) : shiftHoriz(e, e.overlapdir, !0);
	else if (e.overlaps.length > 0) {
			var t, n, x = e.overlaps, r = {
					right: -1 / 0
			}, i = {
					left: 1 / 0
			}, a = 0;
			for (n in e.overlapfix = !0,
			x)
					a += getMidX(t = x[n]),
					t.right > r.right && (r = t),
					t.left < i.left && (i = t);
			a /= x.length,
			getMidX(e) >= a - unitsized16 ? (e.overlapdir = unitsize,
			e.ocheck = r) : (e.overlapdir = -unitsize,
			e.ocheck = i)
	}
}
...
function solidOnCharacter(e, t) {
	return !(t.yvel >= 0) && (t.midx = getMidX(t),
	t.midx > e.left && t.midx < e.right && e.bottom - e.yvel <= t.top + t.toly - t.yvel)
}
	...
	if (t.up != e) {
			if (characterOnSolid(e, t)) {
					if (t.hidden)
							return;
					e.resting = t,
					e.player && map.underwater && removeClass(e, "paddling")
					...
					if (n > t.left && n < t.right)
							...
							return;
					e.under ? e.under.push(t) : e.under = [t],
					e.player && setTop(e, t.bottom - e.toly + t.yvel, !0),
					e.yvel = t.yvel,
					e.player && (e.keys.jump = 0)
			}
			t.hidden || characterNotBumping(e, t) || objectOnTop(e, t) || objectOnTop(t, e) || e.under || e == t.up || (e.right <= t.right ? (e.xvel = min(e.xvel, 0),
			shiftHoriz(e, max(t.left + unitsize - e.right, -unitsized2), !0)) : e.left >= t.left && (e.xvel = max(e.xvel, 0),
			shiftHoriz(e, min(t.right - unitsize - e.left, unitsized2), !0)),
			e.player ? t.actionLeft && t.actionLeft(e, t, t.transport) : (e.moveleft = !e.moveleft,
			"item" == e.group && e.collide(t, e)))
	}
}
...
function characterTouchesUp(e, t) {
	switch (e.group) {
	case "item":
			e.moveleft = getMidX(e) <= getMidX(t) + unitsized2,
			characterHops(e);
			break;
	case "coin":
			e.animate(e);
...
function moveSimple(e) {
	e.direction != e.moveleft && (e.moveleft ? (e.xvel = -e.speed,
	e.noflip || unflipHoriz(e)) : (e.noflip || flipHoriz(e),
	e.xvel = e.speed),
	e.direction = e.moveleft)
}
function moveSmart(e) {
	moveSimple(e),
	0 != e.yvel || e.resting && !offResting(e) || (e.moveleft ? shiftHoriz(e, unitsize, !0) : shiftHoriz(e, -unitsize, !0),
...
function moveJumping(e) {
	moveSimple(e),
	e.resting && (e.yvel = -abs(e.jumpheight),
	e.resting = !1)
}
function moveFloating(e) {
	setPlatformEndpoints(e),
	e.begin = map.floor * unitsize - e.begin,
	e.end = map.floor * unitsize - e.end,
	(e.movement = moveFloatingReal)(e)
}
function moveFloatingReal(e) {
	e.top < e.end ? e.yvel = min(e.yvel + unitsized32, e.maxvel) : e.bottom > e.begin && (e.yvel = max(e.yvel - unitsized32, -e.maxvel)),
	movePlatformNorm(e)
}
function moveSliding(e) {
	setPlatformEndpoints(e),
	(e.movement = moveSlidingReal)(e)
}
function moveSlidingReal(e) {
	gamescreen.left + e.left < e.begin ? e.xvel = min(e.xvel + unitsized32, e.maxvel) : gamescreen.left + e.right > e.end && (e.xvel = max(e.xvel - unitsized32, -e.maxvel)),
	movePlatformNorm(e)
}
...
function collideTransport(e, t) {
	characterTouchedSolid(e, t),
	t == e.resting && (t.movement = movePlatformNorm,
	t.collide = characterTouchedSolid,
	t.xvel = unitsized2)
}
function moveFalling(e) {
	if (e != player.resting)
			return e.yvel = 0;
	shiftVert(e, e.yvel += unitsized8),
	setBottom(player, e.top),
	e.yvel >= 2.8 * unitsize && (e.freefall = !0,
	e.movement = moveFreeFalling)
}
function moveFallingScale(e) {
	player.resting == e ? (shiftScaleStringVert(e, e.string, e.yvel += unitsized16),
	shiftScaleStringVert(e.partner, e.partner.string, -e.yvel),
	e.tension += e.yvel,
	e.partner.tension -= e.yvel) : e.yvel > 0 && (shiftScaleStringVert(e, e.string, e.yvel -= unitsized32),
	shiftScaleStringVert(e.partner, e.partner.string, -e.yvel),
	e.tension -= e.yvel,
	e.partner.tension += e.yvel),
	e.partner.tension <= 0 && (e.collide = e.partner.collide = characterTouchedSolid,
	e.movement = e.partner.movement = moveFreeFalling)
}
function moveFreeFalling(e) {
	shiftVert(e, e.yvel += unitsized16),
	e.yvel > unitsizet2 && (e.movement = function(e) {
			shiftVert(e, e.yvel)
	}
	)
}
function shiftScaleStringVert(e, t, n) {
	shiftVert(e, n),
	t.bottom = e.top,
	t.height = (t.bottom - t.top) / unitsize,
	updateSize(t)
}
...
function removeClasses(e) {
	...
	for (n = 1; n < arguments.length; ++n)
			for ((t = arguments[n])instanceof Array || (t = t.split(" ")),
			x = t.length - 1; x >= 0; --x)
					removeClass(e, t[x])
}
function addClasses(e, t) {
	for (var n = t instanceof Array ? t : t.split(" "), x = n.length - 1; x >= 0; --x)
			addClass(e, n[x])
}
...
function killNormal(e) {
	e && (e.hidden = e.dead = !0,
	e.alive = e.resting = e.movement = !1,
	TimeHandler.clearAllCycles(e))
}
function killFlip(e, t) {
	flipVert(e),
	e.bottomBump = function() {}
	,
	e.nocollide = e.dead = !0,
	e.resting = e.movement = e.speed = e.xvel = e.nofall = !1,
	e.yvel = -unitsize,
	TimeHandler.addEvent((function(e) {
			killNormal(e)
	}
...
function blockBumpMovement(e) {
	var t = -3
		, n = setInterval((function() {
			shiftVert(e, t),
			3.5 == (t += .5) && (clearInterval(n),
			e.up = !1),
			determineThingCollisions(e)
	}
	), timer)
}
function emergeUp(e, t) {
	...
	flipHoriz(e),
	e.nomove = e.nocollide = e.alive = e.nofall = e.emerging = !0,
	switchContainers(e, characters, scenery);
	var n = setInterval((function() {
			shiftVert(e, -unitsized8),
			e.bottom <= t.top && (clearInterval(n),
			switchContainers(e, scenery, characters),
			e.nocollide = e.nomove = e.moveleft = e.nofall = e.emerging = !1,
			e.emergeOut && e.emergeOut(e, t),
			e.movement && (e.movementsave = e.movement,
			e.movement = moveSimple,
			e.moving = TimeHandler.addEventInterval((function(e, t) {
					if (e.resting != t)
							return TimeHandler.addEvent((function(e) {
...
function killOtherCharacters() {
	...
	if (window.characters)
			for (t = characters.length - 1; t >= 0; --t)
					(e = characters[t]).nokillend ? e.killonend && e.killonend(e) : deleteThing(e, characters, t);
	if (window.solids)
			for (t = solids.length - 1; t >= 0; --t)
					solids[t].killonend && deleteThing(solids[t], solids, t)
}
function lookTowardPlayer(e, t) {
	player.right <= e.left ? e.lookleft && !t || (e.lookleft = !0,
	e.moveleft = !1,
	unflipHoriz(e)) : player.left >= e.right && (e.lookleft || t) && (e.lookleft = !1,
	e.moveleft = !0,
	flipHoriz(e))
}
function lookTowardThing(e, t) {
	t.right <= e.left ? (e.lookleft = !0,
	e.moveleft = !1,
	unflipHoriz(e)) : t.left >= e.right && (e.lookleft = !1,
	e.moveleft = !0,
	flipHoriz(e))
}
...
!function(e) {
	...
	var t = function() {}
		...
		, x = function(e) {
			var n = this
				, x = window;
			this.update = t,
			this.requestAnimationFrame = e || x.requestAnimationFrame || x.webkitRequestAnimationFrame || x.mozRequestAnimationFrame,
	...
	i.prototype.update = function() {
			var e, t, n = this, x = Array.prototype.slice.call(this.gamepadGetter(), 0);
			for (t = this.knownGamepads.length - 1; t >= 0; t--)
					e = this.knownGamepads[t],
					x.indexOf(e) < 0 && (this.knownGamepads.splice(t, 1),
					this.listener._disconnect(e));
			for (t = 0; t < x.length; t++)
					(e = x[t]) && n.knownGamepads.indexOf(e) < 0 && (n.knownGamepads.push(e),
					n.listener._connect(e))
	}
	;
	var a = function(e) {
			this.listener = e,
	...
	a.factory = function(e) {
			var t = n;
			return window && void 0 !== window.addEventListener && (t = new a(e)),
			t
	}
	,
	a.getType = function() {
	...
	var o = function(e) {
			this.updateStrategy = e || new x,
			this.gamepads = [],
			this.listeners = {},
			this.platform = n,
			this.deadzone = .03,
			this.maximizeThreshold = .97
	};
	o.UpdateStrategies = {
			AnimFrameUpdateStrategy: x,
			ManualUpdateStrategy: r
	},
	o.PlatformFactories = [i.factory, a.factory],
	o.Type = {
	...
	o.StandardButtons = ["FACE_1", "FACE_2", "FACE_3", "FACE_4", "LEFT_TOP_SHOULDER", "RIGHT_TOP_SHOULDER", "LEFT_BOTTOM_SHOULDER", "RIGHT_BOTTOM_SHOULDER", "SELECT_BACK", "START_FORWARD", "LEFT_STICK", "RIGHT_STICK", "DPAD_UP", "DPAD_DOWN", "DPAD_LEFT", "DPAD_RIGHT", "HOME"],
	o.StandardAxes = ["LEFT_STICK_X", "LEFT_STICK_Y", "RIGHT_STICK_X", "RIGHT_STICK_Y"];
	...
	o.prototype._fire = function(e, t) {
			if (void 0 !== this.listeners[e])
					for (var n = 0; n < this.listeners[e].length; n++)
							this.listeners[e][n].apply(this.listeners[e][n], [t])
	}
	,
	...
	o.resolvePlatform = function(e) {
			var t, x = n;
			for (t = 0; !x.isSupported() && t < o.PlatformFactories.length; t++)
					x = o.PlatformFactories[t](e);
			return x
	}
	,
	o.prototype._connect = function(e) {
			...
			e.lastState = {},
			e.updater = [],
			t = x.buttons.byButton.length,
			n = 0; n < t; n++)
					this._addButtonUpdater(e, x, n);
			for (t = x.axes.byAxis.length,
			n = 0; n < t; n++)
					this._addAxisUpdater(e, x, n);
			this.gamepads[e.index] = e,
			this._fire(o.Event.CONNECTED, e)
	}
	,
	o.prototype._addButtonUpdater = function(e, t, n) {
			var x, r = s(o.StandardButtons, n, "EXTRA_BUTTON_"), i = this._createButtonGetter(e, t.buttons, n), a = this, l = {
					gamepad: e,
					control: r
			};
			e.state[r] = 0,
			...
			x = function() {
					var t = i()
						...
						, s = n > .5;
					e.state[r] = t,
					x && !s ? a._fire(o.Event.BUTTON_DOWN, Object.create(l)) : !x && s && a._fire(o.Event.BUTTON_UP, Object.create(l)),
					0 !== t && 1 !== t && t !== n && a._fireAxisChangedEvent(e, r, t),
					e.lastState[r] = t
			}
			,
			e.updater.push(x)
	...
	o.prototype._addAxisUpdater = function(e, t, n) {
			var x, r = s(o.StandardAxes, n, "EXTRA_AXIS_"), i = this._createAxisGetter(e, t.axes, n), a = this;
			e.state[r] = 0,
			e.lastState[r] = 0,
			x = function() {
					var t = i()
						, n = e.lastState[r];
					e.state[r] = t,
					t !== n && a._fireAxisChangedEvent(e, r, t),
					e.lastState[r] = t
			}
			,
			e.updater.push(x)
	...
	function(e, t, n) {
			...
			) : t.byAxis && n < t.byAxis.length && (x = t.byAxis[n],
			r = x,
			"[object Array]" === Object.prototype.toString.call(r) && 3 == x.length && x[0] < e.axes.length && (i = function(e, t, n) {
					var x = l;
	...
	o.prototype._disconnect = function(e) {
			var t, n = [];
			for (void 0 !== this.gamepads[e.index] && delete this.gamepads[e.index],
			t = 0; t < this.gamepads.length; t++)
					void 0 !== this.gamepads[t] && (n[t] = this.gamepads[t]);
			this.gamepads = n,
			this._fire(o.Event.DISCONNECTED, e)
	}
	,
	...
	o.prototype._resolveMapping = function(e) {
			var t, n, x = o.Mappings, r = null, i = {
					platform: this.platform.getType(),
					type: this._resolveControllerType(e.id)
			};
			for (t = 0; !r && t < x.length; t++)
					n = x[t],
					o.envMatchesFilter(n.env, i) && (r = n);
	...
	o.prototype._update = function() {
			...
			this.gamepads.forEach((function(e) {
	...
	o.prototype._applyDeadzoneMaximize = function(e, t, n) {
			return t = void 0 !== t ? t : this.deadzone,
			n = void 0 !== n ? n : this.maximizeThreshold,
			e >= 0 ? e < t ? e = 0 : e > n && (e = 1) : e > -t ? e = 0 : e < -n && (e = -1),
			e
	}
	,
	e.Gamepad = o
...
function AudioPlayr(e) {
	...
	var t, n, x, r, i, a, o, l, s, d, u = this.play = function(e) {
			...
			if (!n) {
					if (!(n = t[e]))
							return console.log("Unknown sound: '" + e + "'"),
							n;
					x[e] = n
			}
			return n.name_raw = e,
			...
			n.volume = !i,
			customMute || n.play(),
			n.used++ || n.addEventListener("ended", (function() {
					!function(e, t) {
							x[t] && delete x[t]
					}(0, e)
			}
	...
	function m(e, t) {
			...
			n)
					x = n[r],
					i.appendChild(h(document.createElement("Source"), {
							type: "audio/" + x,
							src: a + "/" + t + "/" + x + "/" + e + "." + x
					}));
			return customMute || i.play(),
			i
	}
	function h(e, t) {
			var n, x;
			for (x in t)
					"object" == typeof (n = t[x]) ? (e[x] || (e[x] = {}),
					h(e[x], n)) : e[x] = n;
			return e
	}
	this.playLocal = function(e, t) {
			...
			return x.volume = x.volume_real = n = 0,
			x
	}
	,
	this.playTheme = function(e, t, n) {
			...
			return i.loop = n,
			t || (i.used = !1),
			1 == i.used && i.addEventListener("ended", this.playTheme),
			i
	}
	,
	...
	this.toggleMute = function() {
			for (var e in i = !i,
			x)
					x[e].volume = i ? 0 : x[e].volume_real || 1;
			o && (localStorage[o] = i)
	}
	,
	this.pause = function() {
	...
	function(e) {
			t = e.library || {},
			n = e.filetypes || ["mp3", "ogg"],
			i = e.muted || !1,
			a = e.directory || "",
			o = e.localStorageMuted || "",
			l = e.getVolumeLocal || 1,
			s = e.getThemeDefault || "Theme";
...
function TimeHandlr(e) {
	...
	var d = this.addEvent = function(e, n) {
			if (!(e instanceof Function))
					return console.warn("Attempting to add an event that isn't a function."),
		...
		, p = this.addEventIntervalSynched = function(e, n, x, r, i) {
			var a = n * i.length
				...
				, l = this
				, s = function(e, n, x) {
					return x.startcount = t,
					u.apply(e, n)
			};
			if (n = n || 1,
			x = x || 1,
			o == t)
					return s(l, arguments, r);
			var p = o - t;
			d(s, p, l, arguments, r)
	}
	;
	function c(e, t) {
			return n[t] ? (n[t].push(e),
			n[t]) : n[t] = [e]
	}
	...
	this.addSpriteCycle = function(e, t, n, r) {
			...
			return a.event && i && (a.event.count_changer = r),
			f(e, t),
			a
	}
	,
	...
	function f(e, t) {
			...
			if (null != a && !e[a])
					return !0;
			-1 != t.oldclass && "" !== t.oldclass && s(e, t.oldclass),
			t.loc = ++t.loc % t.length;
			var n = t[t.loc];
			if (n) {
					var x = n instanceof Function ? n(e, t) : n;
					return "string" == typeof x ? (t.oldclass = x,
					l(e, x),
					!1) : !1 === x
			}
			return !1 === n
	...
	this.handleEvents = function() {
			...
			if (e) {
					...
					for (i = 0,
					r = e.length; i < r; ++i)
							(x = e[i]).repeat > 0 && !x.func.apply(this, x.args) && (x.count_changer && x.count_changer(x),
							x.repeat instanceof Function ? x.repeat.bind(x)() && (x.count += x.time_repeat,
							c(x, x.time_exec)) : --x.repeat > 0 && (x.time_exec += x.time_repeat,
							c(x, x.time_exec)));
					delete n[t]
			}
	}
	...
	!function(e) {
			...
			r = e.onSpriteCycleStart || "onSpriteCycleStart",
			i = e.doSpriteCycleStart || "doSpriteCycleStart",
			a = e.cycleCheckValidity,
			o = e.timingDefault || 7,
			l = e.addClass || window.addClass || v,
			s = e.removeClass || window.removeClass || y
	}(e || {})
}
function QuadsKeepr(e) {
	...
	!function(e) {
			...
			i || (i = 2),
			x = r * i),
			a = e.screen_width || 640,
			o = e.screen_height || 480,
			l = a / (i - 3),
			s = o / (r - 2),
			d = e.tolerance || 0,
			u = e.delx || -2 * l,
			p = l,
			h = e.thing_left || "left",
			g = e.thing_right || "right",
			f = e.thing_top || "top",
			v = e.thing_bottom || "bottom",
			y = e.thing_num_quads || "numquads",
			w = e.thing_max_quads || "maxquads",
			T = e.thing_quadrants || "quadrants",
			S = e.onUpdate,
			e.onCollide,
			P()
	}(e || {})
